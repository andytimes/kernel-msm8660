 ...
 */
/*
 * 325 Model Battery Alarm driver
 *	based on Qualcomm PMIC PM8xxx Battery Alarm driver
 *
 */
...
static int b325_battery_gauge_alarm_notify(struct notifier_block *nb,
					  unsigned long status, void *unused);


static struct notifier_block alarm_notifier = {
	.notifier_call = b325_battery_gauge_alarm_notify,
};
...
#ifdef LGE_DEBUG
    if (comparator == PM8XXX_BATT_ALARM_LOWER_COMPARATOR)
        pr_info("battery alarm lower enable\n");
    else
        pr_info("battery alarm upper enable\n");
#endif
...
#ifdef LGE_DEBUG
    if (comparator == PM8XXX_BATT_ALARM_LOWER_COMPARATOR)
        pr_info(" battery alarm lower disable\n");
    else
        pr_info(" battery alarm upper disable\n");
#endif
...
#ifdef LGE_DEBUG
    pr_info(" threshod set value is %d\n", threshold_mV);
#endif
...
int pm8xxx_batt_alarm_config_lge_325(bool enable)
{
	...
	if(lge_bd_rev > LGE_REV_C)
...
#else
			mv = max17047_suspend_get_mvolts();
#endif
		}
	else
		{
		...
		else if (mv > P00_THRESHOLD_LOWER && mv <= P01_THRESHOLD_LOWER)
			threshold_mv = P00_THRESHOLD_LOWER;
		else if (mv > 3450 && mv <= P00_THRESHOLD_LOWER)
			threshold_mv = 3450;
		else
			threshold_mv = 3400;
...
EXPORT_SYMBOL(pm8xxx_batt_alarm_config_lge_325);
#endif
		...
		unsigned long status, void *unused)
{
	...
	case 1:
		// if (!is_chg_plugged_in()) {
			rc = pm8xxx_batt_alarm_disable(PM8XXX_BATT_ALARM_LOWER_COMPARATOR);
			if (rc)
				pr_info("%s: unable to set alarm state\n", __func__);
				//msm_charger_notify_event(NULL, CHG_BATT_NEEDS_RECHARGING);
		 //	}
		break;
	case 2:
		pr_info(	"%s: trip of high threshold\n", __func__);
		break;
	default:
		pr_info(	"%s: error received\n", __func__);
	};
...
/* TODO: Is it best to trigger on both edges? Should this be configurable? */
	...
	if (!is_chg_plugged_in()) {
		pm8xxx_batt_alarm_config_lge_325(1);
	}
...
#if 1
//                                  
static int pm8xxx_batt_alarm_suspend(struct device *dev)
{
    return 0;
}

static int pm8xxx_batt_alarm_resume(struct device *dev)
{
    int soc;
...
#else
			    soc = max17047_get_battery_capacity_percent();
#endif
		}
	else
		{
    ...
    if (soc == 15 || soc == 3 || soc == 1 || soc == 0) {
        ...
        //pm8xxx_batt_alarm_config_defaults();
