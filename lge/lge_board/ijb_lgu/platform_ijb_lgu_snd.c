...
//                                                 
	.hph_spk_on = {		// at&t simultaneously Ringing Headset and SPK
		       ...
		       .amp_function_size =
		       ARRAY_SIZE(seq_lin_to_headset_spkout),
		       },
	...
	.speaker_playback_on = {	// playback
				...
				.amp_function_size =
				ARRAY_SIZE(seq_lin_to_spkout_playback),
				},

	.hph_playback_on = {	// playback
			    .amp_function = seq_lin_to_headset_playback,
			    .amp_function_size =
			    ARRAY_SIZE(seq_lin_to_headset_playback),
			    },
//                                                 
	.power_down = {
	...
	.headset_speaker_pre = {
				...
				.amp_tuning_function =
				seq_lin_to_headset_speaker_pre,
				.amp_function_size =
				ARRAY_SIZE(seq_lin_to_headset_speaker_pre),
				},
#endif
	.set_amp_path = NULL
};
...
#ifdef CONFIG_LGE_AUDIO_AMP_WM9093
	if (lge_audio_wm9093_platform.set_amp_path)
		lge_audio_wm9093_platform.
		    set_amp_path(ICODEC_SPEAKER_PLAYBACK_RX);
#endif
...
static int msm_snddev_poweramp_on_hph_playback(void)
{
	...
	if (lge_audio_wm9093_platform.set_amp_path)
		lge_audio_wm9093_platform.
		    set_amp_path(ICODEC_HEADSET_ST_PLAYBACK_RX);
#endif /*                             */
...
static int msm_snddev_poweramp_on_hph_spk(void)
{
	...
	if (lge_audio_wm9093_platform.set_amp_path)
		lge_audio_wm9093_platform.
		    set_amp_path(ICODEC_HEADSET_ST_RX_SPEAKER_RX);
#endif /*                             */
...
/*                                  */
...
#ifdef CONFIG_PMIC8058_OTHC
	{
		...
		if (ret)
			pr_err
			    ("%s: Disabling OTHC_MICBIAS_MAIN(%d) power failed\n",
			     __func__, OTHC_MICBIAS_MAIN);
	}
#endif
#endif
...
static int msm_snddev_enable_amic_sec_power(void)
{
	...
	if (machine_is_msm8x60_fluid()) {
		...
		ret = gpio_request(SNDDEV_GPIO_HS_MIC4_SEL, "HS_MIC4_SEL");
		if (ret) {
...
static struct adie_codec_action_unit
    ispkr_stereo_playback_48KHz_osr256_actions[] =
...
static struct adie_codec_action_unit
    fluid_dual_mic_endfire_8KHz_osr256_actions[] = FLUID_AMIC_DUAL_8000_OSR_256;
...
static struct adie_codec_hwsetting_entry fluid_dual_mic_endfire_settings[] = {
	{
	 ...
	 .action_sz = ARRAY_SIZE(fluid_dual_mic_endfire_8KHz_osr256_actions),
	 }
};
...
static struct adie_codec_hwsetting_entry fluid_dual_mic_broadside_settings[] = {
	{
	 ...
	 .action_sz = ARRAY_SIZE(fluid_dual_mic_broadside_8KHz_osr256_actions),
	 }
};
...
static struct adie_codec_action_unit dual_nc_48KHz_osr256_actions[] = AMIC_DUAL_OSR_256;	///////////////
...
static struct adie_codec_action_unit mic1_nc_48KHz_osr256_actions[] = AMIC_PRI_MONO_OSR_256;	///////////////
...
static struct adie_codec_action_unit mic2_nc_48KHz_osr256_actions[] = AMIC_SEC_MONO_OSR_256;	///////////////
...
static struct adie_codec_action_unit mic1_acs_48KHz_osr256_actions[] = AMIC_PRI_MONO_OSR_256_ACS;	///////////////
...
static struct adie_codec_action_unit ftm_spkr_l_rx_actions[] = FTM_SPKR_L_RX;
...
static struct adie_codec_action_unit ftm_spkr_r_rx_actions[] = SPKR_R_RX;
...
static struct adie_codec_action_unit ftm_aux_out_rx_actions[] = AUX_OUT_RX;
...
static struct adie_codec_action_unit ftm_spkr_l_rx_lp_actions[] = FTM_SPKR_L_RX;
...
static struct adie_codec_hwsetting_entry ftm_handset_mic_adie_lp_tx_settings[] = {
	{
	 .freq_plan = 48000,
	 .osr = 256,
...
static struct adie_codec_hwsetting_entry ftm_headset_mic_adie_lp_tx_settings[] = {
	{
	 .freq_plan = 48000,
	 .osr = 256,
...
#ifdef CONFIG_DEBUG_FS
static struct adie_codec_action_unit
    ihs_stereo_rx_class_d_legacy_48KHz_osr256_actions[] = HPH_PRI_D_LEG_STEREO;
...
static struct adie_codec_hwsetting_entry
    ihs_stereo_rx_class_d_legacy_settings[] = {
	{
	 .freq_plan = 48000,
	 .osr = 256,
	 .actions = ihs_stereo_rx_class_d_legacy_48KHz_osr256_actions,
...
static struct adie_codec_hwsetting_entry
    ihs_stereo_rx_class_ab_legacy_settings[] = {
	{
	 .freq_plan = 48000,
	 .osr = 256,
	 .actions = ihs_stereo_rx_class_ab_legacy_48KHz_osr256_actions,
	 .action_sz = ARRAY_SIZE
	 (ihs_stereo_rx_class_ab_legacy_48KHz_osr256_actions),
	 }
};
...
//      pr_info("%s [%d 0x%04X]\n", __func__, AMPParamIndex, cal_db);    
	...
	case SPK_IN2_VOL:
		seq_lin_to_spkout[4].wmdata = seq_lin_to_spkout[5].wmdata =
		    cal_db;
		break;
	...
	case HPH_IN1_VOL:
#ifdef CONFIG_LGE_AUDIO_WM9093_POPNOISE
		seq_lin_to_headset[2].wmdata = seq_lin_to_headset[3].wmdata =
		    cal_db;
#else
		seq_lin_to_headset[4].wmdata = seq_lin_to_headset[5].wmdata =
		    cal_db;
#endif
		break;
	...
	case HPH_OUT_VOL:
#ifdef CONFIG_LGE_AUDIO_WM9093_POPNOISE
		seq_lin_to_headset[9].wmdata = seq_lin_to_headset[10].wmdata =
		    cal_db;
#else
		seq_lin_to_headset[11].wmdata = seq_lin_to_headset[12].wmdata =
		    cal_db;
#endif
		break;

	case TTY_IN1_VOL:
		seq_lin_to_headset_tty[4].wmdata =
		    seq_lin_to_headset_tty[5].wmdata = cal_db;
		break;

	case TTY_OUT_VOL:
		seq_lin_to_headset_tty[11].wmdata =
		    seq_lin_to_headset_tty[12].wmdata = cal_db;
		break;

	case HPH_SPK_IN1_VOL:
#ifdef CONFIG_LGE_AUDIO_WM9093_POPNOISE
		seq_lin_to_headset_spkout[2].wmdata =
		    seq_lin_to_headset_spkout[3].wmdata = cal_db;
#else
		seq_lin_to_headset_spkout[4].wmdata =
		    seq_lin_to_headset_spkout[5].wmdata = cal_db;
#endif
		break;
...
#ifdef CONFIG_LGE_AUDIO_WM9093_POPNOISE
		seq_lin_to_headset_spkout[13].wmdata =
		    seq_lin_to_headset_spkout[14].wmdata = cal_db;
#else
		seq_lin_to_headset_spkout[15].wmdata =
		    seq_lin_to_headset_spkout[16].wmdata = cal_db;
#endif
		break;

	case P_SPK_IN2_VOL:
		seq_lin_to_spkout_playback[4].wmdata =
		    seq_lin_to_spkout_playback[5].wmdata = cal_db;
		break;
	...
	case P_HPH_IN1_VOL:
#ifdef CONFIG_LGE_AUDIO_WM9093_POPNOISE
		seq_lin_to_headset_playback[2].wmdata =
		    seq_lin_to_headset_playback[3].wmdata = cal_db;
#else
		seq_lin_to_headset_playback[4].wmdata =
		    seq_lin_to_headset_playback[5].wmdata = cal_db;
#endif
		break;

	case P_HPH_OUT_VOL:
#ifdef CONFIG_LGE_AUDIO_WM9093_POPNOISE
		seq_lin_to_headset_playback[9].wmdata =
		    seq_lin_to_headset_playback[10].wmdata = cal_db;
#else
		seq_lin_to_headset_playback[11].wmdata =
		    seq_lin_to_headset_playback[12].wmdata = cal_db;
#endif
		break;
...
#ifdef CONFIG_LGE_AUDIO_AMP_WM9093
		if (token)	//                                                     
			set_wm9093_cal_db(AMPParamIndex,
					  (u16) strtol(token, NULL, 16));
#endif /*                             */
		AMPParamIndex++;
#ifdef CONFIG_LGE_AUDIO_AMP_WM9093
...
int get_wm9093_cal_db(char *data_array)
{
	...
	if (buff_len + strlen(str) + 4 >= ARRAY_SIZE(str))
		goto END_OF_CODES;
	...
	/* HPH_SPK_IN1_VOL */
#ifdef CONFIG_LGE_AUDIO_WM9093_POPNOISE
	sprintf(str, wm9093_cfg_item[cnt++],
		seq_lin_to_headset_spkout[2].wmdata);
#else
	sprintf(str, wm9093_cfg_item[cnt++],
		seq_lin_to_headset_spkout[4].wmdata);
...
#ifdef CONFIG_LGE_AUDIO_WM9093_POPNOISE
	sprintf(str, wm9093_cfg_item[cnt++],
		seq_lin_to_headset_spkout[6].wmdata);
#else
	sprintf(str, wm9093_cfg_item[cnt++],
		seq_lin_to_headset_spkout[8].wmdata);
#endif
	if (buff_len + strlen(str) + 4 >= ARRAY_SIZE(str))
		goto END_OF_CODES;
	memcpy(&(data_array[buff_len]), str, strlen(str));
...
#ifdef CONFIG_LGE_AUDIO_WM9093_POPNOISE
	sprintf(str, wm9093_cfg_item[cnt++],
		seq_lin_to_headset_spkout[7].wmdata);
#else
	sprintf(str, wm9093_cfg_item[cnt++],
		seq_lin_to_headset_spkout[9].wmdata);
#endif
	if (buff_len + strlen(str) + 4 >= ARRAY_SIZE(str))
		goto END_OF_CODES;
	memcpy(&(data_array[buff_len]), str, strlen(str));
	...
	/* P_SPK_BOOST */
	sprintf(str, wm9093_cfg_item[cnt++],
		seq_lin_to_spkout_playback[10].wmdata);
	if (buff_len + strlen(str) + 4 >= ARRAY_SIZE(str))
		goto END_OF_CODES;
	memcpy(&(data_array[buff_len]), str, strlen(str));
	buff_len += strlen(str);
	...
	/* P_SPK_OUT_VOL */
	sprintf(str, wm9093_cfg_item[cnt++],
		seq_lin_to_spkout_playback[11].wmdata);
	if (buff_len + strlen(str) + 4 >= ARRAY_SIZE(str))
		goto END_OF_CODES;
	memcpy(&(data_array[buff_len]), str, strlen(str));
	buff_len += strlen(str);
...
int audio_subsystem_current_db(void)
{
	...
	if (1000 < data_len) {
		...
		if (filp->f_op->
		    write(filp, data_array, data_len, &filp->f_pos) < 0)
...
void set_qtr8615l_cal_db(int QTR8615LParamIndex, u8 cal_db)
{
	...
	ADIE_CODEC_UNPACK_ENTRY(temp
				[qtr8615l_cal_db[QTR8615LParamIndex].cal_idx].
				action, reg, mask, val);
//    pr_info("Before [reg:0x%02X mask:0x%02X val:0x%02X]\n", reg, mask, val);    

	val = cal_db;

	temp[qtr8615l_cal_db[QTR8615LParamIndex].cal_idx].action =
	    ADIE_CODEC_PACK_ENTRY(reg, mask, val);
...
int qtr8615l_renewal(void)
{
	...
	while ((token = strtok(NULL, separator)) != NULL) {
		token = strtok(NULL, separator);
		if (token)	//                                                     
			set_qtr8615l_cal_db(QTR8615LParamIndex,
					    (u8) strtol(token, NULL, 16));
		QTR8615LParamIndex++;
//        if (QTR8615LParamIndex == QTR8615L_CAL_DB_MAX)
		if (QTR8615LParamIndex >= QTR8615L_CAL_DB_MAX)	//                                                     
			break;
...
int get_qtr8615l_cal_db(char *data_array, int *data_len)
{
	...
	for (cnt = 0; cnt < QTR8615L_CAL_DB_MAX; cnt++) {
		...
		ADIE_CODEC_UNPACK_ENTRY(temp[qtr8615l_cal_db[cnt].cal_idx].
					action, reg, mask, val);
...
static void fsa8008_set_headset_mic_bias(int enable)
{
	...
	if (enable)
		rc = pm8058_micbias_enable(OTHC_MICBIAS_HEADSET,
					   OTHC_SIGNAL_ALWAYS_ON);
	else
		rc = pm8058_micbias_enable(OTHC_MICBIAS_HEADSET,
					   OTHC_SIGNAL_OFF);
...
void __init msm_snddev_init(void)
{
	...
	platform_add_devices(snd_devices_lge_i_board,
			     ARRAY_SIZE(snd_devices_lge_i_board));

#ifdef CONFIG_DEBUG_FS
	debugfs_hsed_config = debugfs_create_file("msm_hsed_config",
						  S_IFREG | S_IRUGO, NULL,
						  (void *)"msm_hsed_config",
						  &snddev_hsed_config_debug_fops);

//                                                                       
	debugfs_wm9093_cal_tool_config =
	    debugfs_create_file(LGE_WM9093_CALIBRATION_TOOL, S_IFREG | S_IRUGO,
				NULL, (void *)LGE_WM9093_CALIBRATION_TOOL,
				&lge_wm9093_cal_tool_fops);

	debugfs_qtr8616l_cal_tool_config =
	    debugfs_create_file(LGE_QTR8615L_CALIBRATION_TOOL,
				S_IFREG | S_IRUGO, NULL,
				(void *)LGE_QTR8615L_CALIBRATION_TOOL,
				&lge_qtr8615l_cal_tool_fops);
//                                                                       
#endif
}
