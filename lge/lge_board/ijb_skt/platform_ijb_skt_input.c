...
static int power_set_for_8058_l11(unsigned char onoff) 
{
			    ...
			    ("%s: line: %d, unable to set pm8058_l11 voltage to 3.0 V\n",
			     __func__, __LINE__);
...
static int power_set_for_8058_l15(unsigned char onoff) 
{
		...
		printk(KERN_INFO "don't need to handle %s, onoff; %d",
			__func__, onoff);
			    ...
			    ("%s: line: %d, unable to set pm8058_l15 voltage to 2.85 V\n",
			     __func__, __LINE__);
...
static unsigned sensor_int_gpio[] =
    { GPIO_GYRO_INT, GPIO_3AXIS_INT, GPIO_COMPASS_INT };
static unsigned sensor_config_power_on[] =
    { GPIO_CFG(GPIO_GYRO_INT, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL,
		GPIO_CFG_2MA), GPIO_CFG(GPIO_3AXIS_INT, 0, GPIO_CFG_INPUT,
...
static unsigned sensor_config_power_off[] =
    { GPIO_CFG(GPIO_GYRO_INT, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN,
		GPIO_CFG_2MA), GPIO_CFG(GPIO_3AXIS_INT, 0, GPIO_CFG_INPUT,
...
void __init sensor_power_init(void) 
{
		    ...
		    gpio_tlmm_config(sensor_config_power_on[i],
				     GPIO_CFG_ENABLE);
...
static int sensor_common_power_set(unsigned char on, int sensor) 
{
		...
		sensor_pwr_mask, on, sensor);
					...
					"%s, power on, pwr_mask=%d, sensor=%d\n",
					__func__, sensor_pwr_mask, sensor);
						...
						"%s, power off, pwr_mask=%d, sensor=%d\n",
						__func__, sensor_pwr_mask,
						sensor);
...
#define APDS9900_ADDRESS 0x39
      ...
      MSM_GPIO_TO_INT(GPIO_PROXIMITY_OUT_INT), .platform_data =
      &proximity_pdata, }, 
};
