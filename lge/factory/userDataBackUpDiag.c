...
extern int lge_write_block(unsigned int bytes_pos, unsigned char *buf,
			    size_t size);
extern int lge_read_block(unsigned int bytes_pos, unsigned char *buf,
			   size_t size);
    ...
    boolean userDataBackUpBadCalc(void) 
{
	...
	int i;
	    ...
	    flash_nand_partition_start_block(MIBIB_DIDBACKUP_PARTI_NAME);
...
 boolean userDataBackUpPartitionErase(void)
{
	    ...
	    lge_erase_block(srd_bytes_pos_in_emmc,
			    (size_t) (SRD_1MBYTE_AREA *
				      SRD_FIRST_AREA_INPARTITION));
	    ...
	    mtd_op_result =
	    lge_erase_block(srd_bytes_pos_in_emmc + 0x400000,
			    (size_t) (SRD_1MBYTE_AREA));
	...
	int i;
	    ...
	    flash_nand_partition_start_block(MIBIB_DIDBACKUP_PARTI_NAME);
			...
			if (nand_device->erase_block(nand_device, i) !=
			     FS_DEVICE_DONE)
...
 boolean writeBackUpCommand(script_process_type MDM_MODEM) 
{
		    ...
		    lge_write_block(srd_bytes_pos_in_emmc,
				    (char *)&userDataBackUpInfo,
				    sizeof(user_data_backup_info));
	
	else
		mtd_op_result =
		    lge_write_block(srd_bytes_pos_in_emmc + 0x400000,
				    (char *)&userDataBackUpInfo,
				    sizeof(user_data_backup_info));
...
 boolean writeBackUpNVdata(char *ram_start_address, unsigned int size)
{
	    ...
	    lge_write_block(srd_bytes_pos_in_emmc +
			    (write_counter_sector * 512), buf, size);
...
===========================================================================*/ 
void diag_userDataBackUp_entrySet(udbp_req_type * req_pkt,
				  udbp_rsp_type * rsp_pkt,
				  script_process_type MODEM_MDM) 
{
	userDataBackUpInfo.info.backup_used =
	    req_pkt->req_data.do_dl_entry.backup_used;
	userDataBackUpInfo.info.binary_class =
	    req_pkt->req_data.do_dl_entry.binary_class;
	userDataBackUpInfo.info.factory_reset_required =
	    req_pkt->req_data.do_dl_entry.factory_reset_required;
	userDataBackUpInfo.info.device_srd_reset_required =
	    req_pkt->req_data.do_dl_entry.device_srd_reset_required;
	userDataBackUpInfo.info.information =
	    req_pkt->req_data.do_dl_entry.information;
	userDataBackUpInfo.info.srd_backup_nv_counter =
	    rsp_pkt->rsp_data.write_sector_counter;
...
===========================================================================*/ 
PACKED void *diag_userDataBackUp_handler(PACKED void *req_pkt_ptr,
					 uint16 pkt_len) 
{
	    ...
	    (udbp_rsp_type *) diagpkt_alloc(DIAG_USET_DATA_BACKUP, rsp_len);
			    ...
			    (void *)diagpkt_err_rsp(DIAG_BAD_PARM_F, req_ptr,
						    pkt_len);
