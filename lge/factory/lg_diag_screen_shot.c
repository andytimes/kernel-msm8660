				   ...
				   struct fb_var_screeninfo *fb_varinfo_p)
{
	...
	for (y = 0; y < fb_varinfo_p->yres; y++) {
		start_pos =
		    y * fb_varinfo_p->xres * (fb_varinfo_p->bits_per_pixel / 8);
		src_buf_ptr = (byte *) src_buf + start_pos;

		for (x = 0;
		     x <
		     fb_varinfo_p->xres * (fb_varinfo_p->bits_per_pixel / 8);
		     x += (fb_varinfo_p->bits_per_pixel / 8)) {
...
//  blt_mode_enable();
	...
	phMscd_Filp->f_op->read(phMscd_Filp, pBuf, LCD_BUFFER_SIZE,
				&phMscd_Filp->f_pos);
...
PACK(void *)LGF_ScreenShot(PACK(void *)req_pkt_ptr,	/* pointer to request packet  */
			   uint16 pkt_len)
{				/* length of request packet   */
	...
	case SCREEN_SHOT_LCD_BUF:
		...
		case SEQ_START:
			...
			rsp_ptr =
			    (diag_screen_shot_type *)
			    diagpkt_alloc(DIAG_LGF_SCREEN_SHOT_F,
					  rsp_len - SCREEN_SHOT_PACK_LEN);
			...
			lcd_buf_info.is_fast_mode =
			    req_ptr->lcd_buf.is_fast_mode;
			...
			lcd_buf_info.total_bytes =
			    lcd_buf_info.width * lcd_buf_info.height * 2;
			...
			vmalloc_buf =
			    vmalloc(LCD_MAIN_WIDTH * LCD_MAIN_HEIGHT * 2);
			vmalloc_buftmp =
			    vmalloc(LCD_MAIN_WIDTH * LCD_MAIN_HEIGHT * 4);
		...
		case SEQ_GET_BUF:
			if (lcd_buf_info.updated == TRUE) {
				...
				rsp_ptr->lcd_buf.total_bytes =
				    lcd_buf_info.total_bytes;
				rsp_ptr->lcd_buf.sended_bytes =
				    lcd_buf_info.sended_bytes;
				rsp_ptr->lcd_buf.packed = FALSE;
				rsp_ptr->lcd_buf.is_fast_mode =
				    lcd_buf_info.is_fast_mode;
				rsp_ptr->lcd_buf.full_draw =
				    lcd_buf_info.full_draw;

				if (lcd_buf_info.total_bytes <
				    SCREEN_SHOT_PACK_LEN) {
					// completed
					lcd_buf_info.sended_bytes = 0;
					rsp_ptr->lcd_buf.sended_bytes =
					    lcd_buf_info.sended_bytes;
					...
					memcpy((void *)
					       &(rsp_ptr->lcd_buf.buf[0]),
					       (void *)&(lcd_buf_info.
							 buf[lcd_buf_info.
							     sended_bytes]),
					       lcd_buf_info.total_bytes);
					rsp_ptr->lcd_buf.seq_flow =
					    SEQ_GET_BUF_COMPLETED;
					lcd_buf_info.update = TRUE;
					lcd_buf_info.updated = FALSE;
				} else if (lcd_buf_info.total_bytes <=
					   lcd_buf_info.sended_bytes) {
					// completed
					lcd_buf_info.sended_bytes -=
					    SCREEN_SHOT_PACK_LEN;
