...
boolean diag_Flag_Data_Init(void) 
{
	    ...
	    /* ==========> eMMC data read   ============ */ 
	    mtd_op_result =
	    lge_read_block(web_status_bytes_pos_in_emmc + select_sector,
			   si_page_buffer, BACKUP_BYTE_MAX_SIZE);
	 if (mtd_op_result != BACKUP_BYTE_MAX_SIZE)
		 {
		printk
		    ("%s, emmc Read error, normal return value size = 2048 : %d\n",
...
 PACK(void *)LGE_WebDload_SRD(PACK(void *)req_pkt_ptr, uint16 pkg_len) 
{
	...
	 webdload_common_cmd((byte *) req_ptr, (byte *) rsp_ptr,
			       CONNECT_MODE_DIAG);
...
void webdload_common_cmd  ( byte * cmd_buf,  byte * rsp_buf,  byte mode )  {
	...
	int batt_state = 0;
	...
	 webdload_memset((byte *) rsp_buf, 0,
			   (int)sizeof(diag_webdload_rsp_type));
		...
		rsp_ptr->rsp_data.current_step =
		    (word) ((status_info_type *) si_page_buffer)->current_step;
	...
	case WEBDLOAD_READ_CNT:
		rsp_ptr->rsp_data.webdload_cnt =
		    (word) ((status_info_type *) si_page_buffer)->webdload_cnt;
		break;
	 case WEBDLOAD_READ_BIN_VER:
		memcpy((void *)rsp_ptr->rsp_data.bin_ver,
			(void *)&((status_info_type *) si_page_buffer)->bin_ver,
			VER_NAME_LEN);
		break;
	 case WEBDLOAD_READ_SUB_REV:
		memcpy((void *)rsp_ptr->rsp_data.sub_rev,
			(void *)&((status_info_type *) si_page_buffer)->sub_rev,
			SUB_REV_LEN);
		break;
	 case WEBDLOAD_READ_RSVD_VAL1:
		rsp_ptr->rsp_data.reserved_val1 =
		    (word) ((status_info_type *) si_page_buffer)->reserved_val1;
		break;
	 case WEBDLOAD_READ_RSVD_VAL2:
		rsp_ptr->rsp_data.reserved_val2 =
		    (word) ((status_info_type *) si_page_buffer)->reserved_val2;
		break;
	 case WEBDLOAD_READ_RSVD_ARRAY1:
		memcpy((void *)rsp_ptr->rsp_data.reserved_array1,
			(void *)&((status_info_type *) si_page_buffer)->
			reserved_array1, RSVD_ARRAY_LEN);
		break;
	 case WEBDLOAD_READ_RSVD_ARRAY2:
		memcpy((void *)rsp_ptr->rsp_data.reserved_array2,
			(void *)&((status_info_type *) si_page_buffer)->
			reserved_array2, RSVD_ARRAY_LEN);
		break;
	 case WEBDLOAD_READ_LANGUAGE:
		rsp_ptr->rsp_data.language =
		    (word) ((status_info_type *) si_page_buffer)->language;
		    ...
		    /* ==========> eMMC data erase   ============ */ 
		    mtd_op_result =
		    lge_erase_block(web_status_bytes_pos_in_emmc +
				    select_sector,
				    (size_t) BACKUP_BYTE_MAX_SIZE);
		 if (mtd_op_result != (BACKUP_BYTE_MAX_SIZE))
			 {
			printk
			    ("%s, emmc erase error, normal return value size = 2048 : %d\n",
			     __func__, mtd_op_result);
		...
		rsp_ptr->rsp_data.auth_mark =
		    (word) ((status_info_type *) si_page_buffer)->auth_mark;
...
//[START] 2011.09.08 jongan.kim - For LTE Model Check
	case WEBDLOAD_QUALCOMM_LTE_TYPE_CHECK:
		memcpy((void *)&rsp_ptr->rsp_data.magic_string,
			LTE_MAGIC_STRING, LTE_MAGIC_STRING_LEN);
		break;
		
//[END]
	case WEBDLOAD_SET_AUTH_MARK:
	case WEBDLOAD_WRITE_STEP_END:
	case WEBDLOAD_WRITE_CNT:
	case WEBDLOAD_WRITE_BIN_VER:
	case WEBDLOAD_WRITE_SUB_REV:
	case WEBDLOAD_WRITE_LANGUAGE:
	case WEBDLOAD_WRITE_RSVD_VAL1:
	case WEBDLOAD_WRITE_RSVD_VAL2:
	case WEBDLOAD_WRITE_RSVD_ARRAY1:
	case WEBDLOAD_WRITE_RSVD_ARRAY2:
	case WEBDLOAD_DL_RESET:
		 {
			if (sub_cmd == WEBDLOAD_SET_AUTH_MARK)
				 {
				((status_info_type *) si_page_buffer)->
				    auth_mark =
				    (word) (sib_step_type) req_ptr->req_data.
				    auth_mark;
				    ...
				    webdload_cnt =
				    (word) req_ptr->req_data.webdload_cnt;
				    ...
				    language =
				    (word) req_ptr->req_data.language;
				    ...
				    /* ==========> eMMC data write   ============ */ 
				    mtd_op_result =
				    lge_write_block(web_status_bytes_pos_in_emmc
						    + select_sector,
						    si_page_buffer,
						    BACKUP_BYTE_MAX_SIZE);
				 if (mtd_op_result != (BACKUP_BYTE_MAX_SIZE))
					 {
					printk
					    ("%s, emmc write error, normal return value size = 2048 : %d\n",
					     __func__, mtd_op_result);
	...
	case WEBDLOAD_READ_BL_VER:
		memcpy((void *)rsp_ptr->rsp_data.bin_ver, (void *)cs_ver_dir,
			CS_VER_LEN);
		webdload_memset((byte *) & rsp_ptr->rsp_data.
				 bin_ver[CS_VER_LEN], 0,
				 (VER_NAME_LEN - CS_VER_LEN));
	...
	case WEBDLOAD_GET_MEID:	/*0x31 */
		    ...
		    rsp_ptr->rsp_data.meid[0] =
		    ((status_info_type *) si_page_buffer)->meid[0];
		rsp_ptr->rsp_data.meid[1] =
		    ((status_info_type *) si_page_buffer)->meid[1];
	...
	case 0x30:		/* We don't use command But other device use command, so we don't delete command.  */
			...
			LTEModemTypecheck * rsp_ptr =
			    (LTEModemTypecheck *) rsp_buf;
			...
			memcpy((void *)&((status_info_type *) si_page_buffer)->
				model_name, (void *)name_req_ptr->model_name,
				MODEL_NAME_LEN);
