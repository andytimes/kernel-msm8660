...
extern void diag_userDataBackUp_entrySet(udbp_req_type * req_pkt,
					 udbp_rsp_type * rsp_pkt,
					 script_process_type MODEM_MDM);
...
void *LGF_TestOTPBlowCommand(test_mode_req_type * pReq,
			     DIAG_TEST_MODE_F_rsp_type * pRsp);
void *LGF_TestWVProvisioningCommand(test_mode_req_type * pReq,
				    DIAG_TEST_MODE_F_rsp_type * pRsp);
void *LGF_TestPRProvisioningCommand(test_mode_req_type * pReq,
				    DIAG_TEST_MODE_F_rsp_type * pRsp);
#endif

#if defined(CONFIG_MACH_LGE_I_BOARD_DCM) || defined(CONFIG_MACH_LGE_325_BOARD_DCM)
void *LGF_TestModeFelica(test_mode_req_type * pReq,
			 DIAG_TEST_MODE_F_rsp_type * pRsp);
#endif
...
void CheckHWRev(byte * pStr)
{
	...
	strcpy((char *)pStr,
	       (char *)rev_str[lge_bd_rev - 9 /*CAYMAN_I_DIFF_VALUE */ ]);
#else
...
{				/* length of request packet */
	...
	case TEST_MODE_TEST_SCRIPT_MODE:
		rsp_len =
		    sizeof(DIAG_TEST_MODE_F_rsp_type) -
		    sizeof(test_mode_rsp_type) +
		    sizeof(test_mode_req_test_script_mode_type);
		break;

		//REMOVE UNNECESSARY RESPONSE PACKET FOR EXTERNEL SOCKET ERASE
	case TEST_MODE_EXT_SOCKET_TEST:
		if ((req_ptr->test_mode_req.esm == EXTERNAL_SOCKET_ERASE)
		    || (req_ptr->test_mode_req.esm ==
			EXTERNAL_SOCKET_ERASE_SDCARD_ONLY)
		    || (req_ptr->test_mode_req.esm ==
			EXTERNAL_SOCKET_ERASE_FAT_ONLY))
			rsp_len =
			    sizeof(DIAG_TEST_MODE_F_rsp_type) -
			    sizeof(test_mode_rsp_type);
		else
	...
	case TEST_MODE_XO_CAL_DATA_COPY:
		rsp_len =
		    sizeof(DIAG_TEST_MODE_F_rsp_type) -
		    sizeof(test_mode_rsp_type) +
		    sizeof(test_mode_req_XOCalDataBackup_Type);
		break;
	...
	case TEST_MODE_MANUAL_TEST_MODE:
		rsp_len =
		    sizeof(DIAG_TEST_MODE_F_rsp_type) -
		    sizeof(test_mode_rsp_type) +
		    sizeof(test_mode_req_manual_test_mode_type);
		break;
	...
	case TEST_MODE_BLUETOOTH_RW:
		rsp_len =
		    sizeof(DIAG_TEST_MODE_F_rsp_type) -
		    sizeof(test_mode_rsp_type) +
		    sizeof(test_mode_req_bt_addr_type);
		break;
	...
	case TEST_MODE_WIFI_MAC_RW:
		rsp_len =
		    sizeof(DIAG_TEST_MODE_F_rsp_type) -
		    sizeof(test_mode_rsp_type) +
		    sizeof(test_mode_req_wifi_addr_type);
		break;
	...
	if (func_ptr != NULL)
		...
		memset(rsp_ptr->test_mode_rsp.str_buf, 0,
		       sizeof(rsp_ptr->test_mode_rsp.str_buf));
		if (req_ptr->test_mode_req.version == VER_HW)
			...
			get_touch_ts_fw_version((byte *) rsp_ptr->test_mode_rsp.
						str_buf);
		else if (req_ptr->test_mode_req.version == VER_SMART_OS)
			get_smartphone_os((byte *) rsp_ptr->test_mode_rsp.
					  str_buf);
		else
...
void *LGF_TestModeMIMO_RFMode(test_mode_req_type * pReq,
			      DIAG_TEST_MODE_F_rsp_type * pRsp)
{
#if 0
	...
	if (diagpdev != NULL) {
		update_diagcmd_state(diagpdev, "LTEMODESELETION",
				     pReq->mode_seletion);
	} else {
...
void *LGF_TestModeLTE_Call(test_mode_req_type * pReq,
			   DIAG_TEST_MODE_F_rsp_type * pRsp)
{
#if 0
	pRsp->ret_stat_code = TEST_OK_S;
...
void *LGF_TestModeDetach(test_mode_req_type * pReq,
			 DIAG_TEST_MODE_F_rsp_type * pRsp)
{
	...
	if (pReq->lte_virtual_sim == 21) {
		...
		if (diagpdev != NULL) {
			update_diagcmd_state(diagpdev, "LTECALLDETACH",
					     pReq->lte_virtual_sim);
		} else {
		...
		if (wbuf) {
			if ((ret =
				...
				printk(KERN_ERR
				       ...
				       __FUNCTION__, length, filename, ret);
				break;
			}
		} else {
			if ((ret =
...
#if defined(CONFIG_MACH_LGE_120_BOARD)	//
static int sd_status_show(struct device *dev, struct device_attribute *attr,
			  char *buf)
{
	return sprintf(buf, "%d\n", g_sd_status);
}

static int sd_status_store(struct device *dev, struct device_attribute *attr,
...
char external_memory_copy_test(void)
{
	...
	if (g_sd_status == 0) {
		printk(KERN_ERR
		       "[Testmode Memory Test] SD card does not mounted\n");
		return return_value;
	}
#endif
	set_fs(get_ds());
	...
	if ((fd =
	     sys_open((const char __user *)EXT_SD_WR_TEST_FILE,
		      O_CREAT | O_RDWR, 0)) < 0) {
		printk(KERN_ERR
		       "[Testmode Memory Test] Can not access SD card\n");
		goto file_fail;
	}
	...
	if ((src = kmalloc(10, GFP_KERNEL))) {
		...
		if ((sys_write(fd, (const char __user *)src, 5)) < 0) {
			printk(KERN_ERR
			       "[Testmode Memory Test] Can not write SD card \n");
			goto file_fail;
		}
	...
	if ((dest = kmalloc(10, GFP_KERNEL))) {
		if ((sys_read(fd, (char __user *)dest, 5)) < 0) {
			printk(KERN_ERR
			       "[Testmode Memory Test] Can not read SD card \n");
			goto file_fail;
		}
			       ...
			       DIAG_TEST_MODE_F_rsp_type * pRsp)
{
	...
	if (external_memory_copy_test()) {
		...
		printk(KERN_ERR
		       "[Testmode Memory Test] Can not detect SD card\n");
		return pRsp;
	}
	...
	case EXTERNAL_FLASH_MEMORY_SIZE:
		if (sys_statfs(EXT_SD_MOUNT_POINT, (struct statfs *)&sf) != 0) {
			printk(KERN_ERR
			       "[Testmode Memory Test] can not get sdcard infomation \n");
			pRsp->ret_stat_code = TEST_FAIL_S;
			break;
		}
	...
	case EXTERNAL_SOCKET_ERASE:
		...
		if (testmode_result[0] != 0xFF) {
			...
			pRsp->ret_stat_code = TEST_FAIL_S;
			printk(KERN_ERR
			       "[MMCFORMAT] DiagCommandObserver returned fail or didn't return in 30000ms.\n");
		}
	...
	case EXTERNAL_FLASH_MEMORY_USED_SIZE:
		...
		if (diagpdev != NULL) {
			...
			printk("\n[%s] error EXTERNAL_FLASH_MEMORY_USED_SIZE",
			       __func__);
			pRsp->ret_stat_code = TEST_FAIL_S;
			break;
		}
		...
		if (external_memory_test != -1) {
			...
			pRsp->ret_stat_code = TEST_FAIL_S;
			printk(KERN_ERR
			       "[CALCUSEDSIZE] DiagCommandObserver returned fail or didn't return in 5000ms.\n");
		}
	...
	case EXTERNAL_FLASH_MEMORY_CONTENTS_CHECK:
		...
		if (diagpdev != NULL) {
			...
			printk
			    ("\n[%s] error EXTERNAL_FLASH_MEMORY_CONTENTS_CHECK",
			     __func__);
			pRsp->ret_stat_code = TEST_FAIL_S;
			break;
		}
		...
		if (external_memory_test != -1) {
			...
			pRsp->ret_stat_code = TEST_FAIL_S;
			printk(KERN_ERR
			       "[CHECKCONTENTS] DiagCommandObserver returned fail or didn't return in 5000ms.\n");
		}
	...
	case EXTERNAL_FLASH_MEMORY_ERASE:
		...
		if (diagpdev != NULL) {
			...
			printk("\n[%s] error EXTERNAL_FLASH_MEMORY_ERASE",
			       __func__);
			pRsp->ret_stat_code = TEST_FAIL_S;
			break;
		}
		...
		if (external_memory_test != -1) {
			...
			pRsp->ret_stat_code = TEST_FAIL_S;
			printk(KERN_ERR
			       "[ERASEMEMORY] DiagCommandObserver returned fail or didn't return in 5000ms.\n");
		}
	...
	case EXTERNAL_SOCKET_ERASE_SDCARD_ONLY:	/*0xE */
		if (diagpdev != NULL) {
			update_diagcmd_state(diagpdev, "MMCFORMAT",
					     EXTERNAL_SOCKET_ERASE_SDCARD_ONLY);
			msleep(5000);
			pRsp->ret_stat_code = TEST_OK_S;
		} else {
			printk("\n[%s] error EXTERNAL_SOCKET_ERASE_SDCARD_ONLY",
			       __func__);
			pRsp->ret_stat_code = TEST_FAIL_S;
		}
		break;
	...
	case EXTERNAL_SOCKET_ERASE_FAT_ONLY:	/*0xF */
		if (diagpdev != NULL) {
			...
			printk("\n[%s] error EXTERNAL_SOCKET_ERASE_FAT_ONLY",
			       __func__);
			pRsp->ret_stat_code = TEST_FAIL_S;
		}
		break;
...
void *LGF_TestModeBattLevel(test_mode_req_type * pReq,
			    DIAG_TEST_MODE_F_rsp_type * pRsp)
{
...
//                                                                                  
	else if (pReq->batt == BATTERY_FUEL_GAUGE_SOC_NPST_DLOAD_TOOL)
//                                                                                  
	{
#ifdef CONFIG_BATTERY_MAX17040
		battery_soc = (int)max17040_get_battery_capacity_percent();
...
#if defined(CONFIG_MACH_LGE_325_BOARD_VZW)
//                                                                                  
	if (pReq->batt == BATTERY_FUEL_GAUGE_SOC_NPST_DLOAD_TOOL) {
		sprintf((char *)pRsp->test_mode_rsp.batt_voltage, "%d",
			battery_soc);
		printk(KERN_ERR "%s, battery_soc : %s\n", __func__,
		       (char *)pRsp->test_mode_rsp.batt_voltage);
	} else {
...
#endif
void *LGF_TestModeKeyData(test_mode_req_type * pReq,
			  DIAG_TEST_MODE_F_rsp_type * pRsp)
{
...
void *LGF_TestModeSleepMode(test_mode_req_type * pReq,
			    DIAG_TEST_MODE_F_rsp_type * pRsp)
{
	char *envp[3];
...
void *LGF_TestModeVirtualSimTest(test_mode_req_type * pReq,
				 DIAG_TEST_MODE_F_rsp_type * pRsp)
{
...
void *LGF_TestModeFBoot(test_mode_req_type * pReq,
			DIAG_TEST_MODE_F_rsp_type * pRsp)
{
	switch (pReq->fboot) {
	...
	if (diagpdev != NULL) {
		...
		case DB_INTEGRITY_CHECK:
			...
			if (strlen(integrity_ret.ret) == 8)
				crc_val =
				    (unsigned int)simple_strtoul(integrity_ret.
								 ret, NULL, 16);
			else
				crc_val =
				    (unsigned int)simple_strtoul(integrity_ret.
...
void *LGF_TestModeFOTA(test_mode_req_type * pReq,
		       DIAG_TEST_MODE_F_rsp_type * pRsp)
{
	int i;
	...
	if (diagpdev != NULL) {
		...
		case FOTA_ID_READ:
			...
			for (i = 0; i < 19; i++)
				pRsp->test_mode_rsp.fota_id_read[i] =
				    fota_id_read[i];

			printk(KERN_ERR "%s, rsp.read_fota_id : %s\n", __func__,
			       (char *)pRsp->test_mode_rsp.fota_id_read);
			pRsp->ret_stat_code = TEST_OK_S;
			break;
...
//                                                                         
void *LGF_TestModeWiFiMACRW(test_mode_req_type * pReq,
			    DIAG_TEST_MODE_F_rsp_type * pRsp)
{
	int fd = 0;
	int i = 0;
		...
		if (pReq->wifi_mac_ad.req_type == 0) {
			...
			if ((src = kmalloc(20, GFP_KERNEL))) {
				sprintf(src, "%c%c%c%c%c%c%c%c%c%c%c%c",
					pReq->wifi_mac_ad.wifi_mac_addr[0],
					pReq->wifi_mac_ad.wifi_mac_addr[1],
					pReq->wifi_mac_ad.wifi_mac_addr[2],
					pReq->wifi_mac_ad.wifi_mac_addr[3],
					pReq->wifi_mac_ad.wifi_mac_addr[4],
					pReq->wifi_mac_ad.wifi_mac_addr[5],
					pReq->wifi_mac_ad.wifi_mac_addr[6],
					pReq->wifi_mac_ad.wifi_mac_addr[7],
					pReq->wifi_mac_ad.wifi_mac_addr[8],
					pReq->wifi_mac_ad.wifi_mac_addr[9],
					pReq->wifi_mac_ad.wifi_mac_addr[10],
					pReq->wifi_mac_ad.wifi_mac_addr[11]
				    );

				if ((sys_write
				     (fd, (const char __user *)src, 12)) < 0) {
					printk(KERN_ERR
					       "[LGF_TestModeWiFiMACRW] Can not write file.\n");
					pRsp->ret_stat_code = TEST_FAIL_S;
					goto file_fail;
				}
#if defined(CONFIG_MACH_LGE_325_BOARD_VZW)
...
//                                                                                
			...
			// check error  If diag_mac_error exist, error was happend in operation.
			if ((fd =
			     sys_open((const char __user *)
				      "/data/misc/diag_mac_error", O_RDWR,
				      0777)) >= 0) {
				printk(KERN_ERR
				       "[LGF_TestModeWiFiMACRW] write operation is failed!!\n");
				sys_unlink((const char __user *)
					   "/data/misc/diag_mac_error");
				printk(KERN_ERR
				       "[LGF_TestModeWiFiMACRW] writing error noti file is removed!!\n");
				pRsp->ret_stat_code = TEST_FAIL_S;
				goto file_fail;
			} else {
				printk(KERN_ERR
				       "[LGF_TestModeWiFiMACRW] writing suceess !!!.\n");
			}
			//--------------------------------------------
//                                                                                
#endif
			...
			printk(KERN_ERR
			       "[LGF_TestModeWiFiMACRW] WIFI_MAC_ADDRESS_READ.\n");
...
//                                                                                
			...
			if ((fd =
			     sys_open((const char __user *)
				      ...
				      0777)) >= 0) {
				printk(KERN_ERR
				       "[LGF_TestModeWiFiMACRW] read operation is failed!!\n");
				pRsp->ret_stat_code = TEST_FAIL_S;
				sys_unlink((const char __user *)
					   "/data/misc/diag_mac_error");
				printk(KERN_ERR
				       "[LGF_TestModeWiFiMACRW] reading error noti file is removed!!\n");
				goto file_fail;
			} else {
				printk(KERN_ERR
				       "[LGF_TestModeWiFiMACRW] reading suceess !!!.\n");
			}
//                                                                                
#endif
			...
			if ((src = kmalloc(20, GFP_KERNEL))) {
				if ((sys_read(fd, (char __user *)src, 12)) < 0) {
					printk(KERN_ERR
					       "[LGF_TestModeWiFiMACRW] Can not read file.\n");
					pRsp->ret_stat_code = TEST_FAIL_S;
					goto file_fail;
				}
			}
			...
			for (i = 0; i < 12; i++) {
				pRsp->test_mode_rsp.read_wifi_mac_addr[i] =
				    src[i];
			...
			printk(KERN_ERR
			       "[LGF_TestModeWiFiMACRW] WIFI_MAC_ADDRESS_READ Result : %c%c%c%c%c%c%c%c%c%c%c%c",
			       pRsp->test_mode_rsp.read_wifi_mac_addr[0],
			       pRsp->test_mode_rsp.read_wifi_mac_addr[1],
			       pRsp->test_mode_rsp.read_wifi_mac_addr[2],
			       pRsp->test_mode_rsp.read_wifi_mac_addr[3],
			       pRsp->test_mode_rsp.read_wifi_mac_addr[4],
			       pRsp->test_mode_rsp.read_wifi_mac_addr[5],
			       pRsp->test_mode_rsp.read_wifi_mac_addr[6],
			       pRsp->test_mode_rsp.read_wifi_mac_addr[7],
			       pRsp->test_mode_rsp.read_wifi_mac_addr[8],
			       pRsp->test_mode_rsp.read_wifi_mac_addr[9],
			       pRsp->test_mode_rsp.read_wifi_mac_addr[10],
...
void *LGF_TestModePowerReset(test_mode_req_type * pReq,
			     DIAG_TEST_MODE_F_rsp_type * pRsp)
{
#if defined(CONFIG_MACH_LGE_325_BOARD)
	...
	if ((diagpdev != NULL) && (deepsleep_result == 0))	// 0: Success
	{
		...
		pRsp->ret_stat_code = TEST_OK_S;
		sprintf((char *)pRsp->test_mode_rsp.str_buf, "%d",
			deepsleep_result);
	} else			// 1: Fail
	{
...
void *LGF_Testmode_ext_device_cmd(test_mode_req_type * pReq,
				  DIAG_TEST_MODE_F_rsp_type * pRsp)
{
	if (diagpdev != NULL) {
...
void *LGF_TestModeKeyLockUnlock(test_mode_req_type * pReq,
				DIAG_TEST_MODE_F_rsp_type * pRsp)
{
	...
	case KEY_LOCK:
		...
		android_readwrite_file
		    ("/sys/class/leds/lcd-backlight/brightness", NULL, buf,
		     len);
#endif
		pRsp->ret_stat_code = TEST_OK_S;
		break;
			       ...
			       DIAG_TEST_MODE_F_rsp_type * pRsp)
{
	...
	printk
	    ("LGF_TestModeFactoryReset> mmc info sec : 0x%x, size : 0x%x type : 0x%x frst sec: 0x%lx\n",
	     pMisc_part->dfirstsec, pMisc_part->dsize, pMisc_part->dtype,
	     factoryreset_bytes_pos_in_emmc);

	switch (pReq->factory_reset) {
	case FACTORY_RESET_CHECK:
#if defined(CONFIG_MACH_LGE_I_BOARD_DCM) || defined(CONFIG_MACH_LGE_325_BOARD_DCM)
		update_diagcmd_state(diagpdev, "FACTORY_RESET_FROM_TESTMODE",
				     1);
#endif

		memset((void *)pbuf, 0, sizeof(pbuf));
		mtd_op_result =
		    lge_read_block(factoryreset_bytes_pos_in_emmc, pbuf,
				   FACTORY_RESET_STR_SIZE + 2);

		if (mtd_op_result != (FACTORY_RESET_STR_SIZE + 2)) {
			printk(KERN_ERR
			       "[Testmode]lge_read_block, read data  = %d \n",
			       mtd_op_result);
			pRsp->ret_stat_code = TEST_FAIL_S;
			break;
		} else {
		...
		mtd_op_result =
		    lge_erase_block(factoryreset_bytes_pos_in_emmc,
				    FACTORY_RESET_STR_SIZE + 2);
		if (mtd_op_result != (FACTORY_RESET_STR_SIZE + 2)) {
			printk(KERN_ERR
			...
			mtd_op_result =
			    lge_write_block(factoryreset_bytes_pos_in_emmc,
					    pbuf, FACTORY_RESET_STR_SIZE + 2);
			if (mtd_op_result != (FACTORY_RESET_STR_SIZE + 2)) {
				printk(KERN_ERR
		...
		if (pRsp->ret_stat_code != TEST_OK_S) {
			printk(KERN_ERR
			       "[Testmode]send_to_arm9 response : %d\n",
			       pRsp->ret_stat_code);
			pRsp->ret_stat_code = TEST_FAIL_S;
			break;
		}
		...
		if ((startStatus == FACTORY_RESET_COLD_BOOT_END)
		    || (startStatus == FACTORY_RESET_HOME_SCREEN_END))
			test_mode_factory_reset_status =
	...
	case FACTORY_RESET_STATUS_CHECK:
		...
		if (mtd_op_result != (FACTORY_RESET_STR_SIZE + 2)) {
			...
			if (memcmp(pbuf, FACTORY_RESET_STR, FACTORY_RESET_STR_SIZE) == 0)	// tag read sucess
			{
				...
				printk(KERN_ERR
				       "[Testmode]factory reset tag fail, set initial state\n");
				test_mode_factory_reset_status =
				    FACTORY_RESET_START;
				pRsp->ret_stat_code =
				    test_mode_factory_reset_status;
	...
	case FACTORY_RESET_FORCE_CHANGE_STATUS:
		...
		sprintf(pbuf, "%s%d", FACTORY_RESET_STR,
			test_mode_factory_reset_status);
		printk(KERN_INFO
		       "[Testmode-force_change]factory reset status = %d\n",
		       test_mode_factory_reset_status);
			    ...
			    DIAG_TEST_MODE_F_rsp_type * pRsp)
{
	...
	req_ptr.test_mode_req.test_mode_test_scr_mode =
	    pReq->test_mode_test_scr_mode;
		...
		if (pReq->test_mode_test_scr_mode == TEST_SCRIPT_MODE_CHECK) {
			...
			default:
				printk(KERN_INFO "%s, mode : %s, returned %d\n",
				       __func__, "NO PRL",
				       pRsp->test_mode_rsp.
				       test_mode_test_scr_mode);
				break;
			}
		}
#else
		pRsp->test_mode_rsp.test_mode_test_scr_mode =
		    pRsp->ret_stat_code;
		pRsp->ret_stat_code = TEST_OK_S;
#endif
#else
...
void *LGF_TestModeMLTEnableSet(test_mode_req_type * pReq,
			       DIAG_TEST_MODE_F_rsp_type * pRsp)
{
	char *src = (void *)0;
	char *dest = (void *)0;
	...
	if (diagpdev != NULL) {
		if ((fd =
		     sys_open((const char __user *)"/mpt/enable",
			      O_CREAT | O_RDWR, 0)) < 0) {
		...
		if ((src = kmalloc(5, GFP_KERNEL))) {
			...
			if ((sys_write(fd, (const char __user *)src, 2)) < 0) {
				printk(KERN_ERR
				       "[Testmode MPT] Can not write MPT \n");
				goto file_fail;
			}
...
typedef PACK(enum)
{
UTS_OK_S, UTS_FAIL_S, UTS_NOT_SUPPORTED_S,} uts_mode_ret_stat_type;
...
PACK(void *) LGE_UTS_CMD(PACK(void *)req_pkt, uint16 pkg_len)
{
	...
	rsp =
	    (lge_tools_response *) diagpkt_alloc(DIAG_LGE_UTS_COMMAND_F,
						 sizeof(lge_tools_response));
...
PACK(void *)LGE_Dload_SRD(PACK(void *)req_pkt_ptr, uint16 pkg_len)
{
	...
	rsp_ptr =
	    (udbp_rsp_type *) diagpkt_alloc(DIAG_USET_DATA_BACKUP, rsp_len);
	...
	case USERDATA_BACKUP_REQUEST:
		remote_rpc_srd_cmmand(req_ptr, rsp_ptr);
		...
		if (load_srd_shard_base == NULL) {
			((udbp_rsp_type *) rsp_ptr)->header.err_code =
			    UDBU_ERROR_CANNOT_COMPLETE;
			break;
			// return rsp_ptr;
		}
	...
	case USERDATA_BACKUP_REQUEST_MDM:
		...
		remote_rpc_srd_cmmand(req_ptr, rsp_ptr);
		diag_userDataBackUp_entrySet(req_ptr, rsp_ptr, 1);
		write_size = rsp_ptr->rsp_data.write_sector_counter * 256;	//return nv backup counters  
...
testmode_user_table_entry_type testmode_mstr_tbl[TESTMODE_MSTR_TBL_SIZE] = {
	...
	,
	/* 11 ~ 20 */
	{TEST_MODE_IRDA_FMRT_FINGER_UIM_TEST, LGF_Testmode_ext_device_cmd,
	 ARM11_PROCESSOR}
	,
	/* 21 ~ 30 */
	...
	,
	{TEST_MODE_BLUETOOTH_TEST, not_supported_command_handler,
	 ARM11_PROCESSOR}
	,
#ifdef CONFIG_BATTERY_MAX17040	//
...
#endif
	/* 31 ~ 40 */
	{TEST_MODE_ACCEL_SENSOR_TEST, not_supported_command_handler,
	 ARM11_PROCESSOR}
	,
#if defined(CONFIG_MACH_LGE_325_BOARD)	//
	...
	,
	{TEST_MODE_FORMAT_MEMORY_TEST, not_supported_command_handler,
	 ARM11_PROCESSOR}
	,
	...
	/* 41 ~ 50 */
#if defined(CONFIG_MACH_LGE_325_BOARD) || defined(CONFIG_MACH_LGE_IJB_BOARD_LGU) || defined (CONFIG_MACH_LGE_IJB_BOARD_SKT)
	{TEST_MODE_MEMORY_CAPA_TEST, not_supported_command_handler,
	 ARM11_PROCESSOR}
	,
#else // FOR CAYMAN
	...
	,
#if defined(CONFIG_MACH_LGE_325_BOARD) || defined(CONFIG_MACH_LGE_IJB_BOARD_LGU) || defined (CONFIG_MACH_LGE_IJB_BOARD_SKT)
	{TEST_MODE_SPEAKER_PHONE_TEST, not_supported_command_handler,
	 ARM11_PROCESSOR}
	,
#else // FOR CAYMAN
	...
	,
	{TEST_MODE_PHOTO_SENSER_TEST, not_supported_command_handler,
	 ARM11_PROCESSOR}
	,
	{TEST_MODE_MRD_USB_TEST, NULL, ARM9_PROCESSOR}
	,
	{TEST_MODE_PROXIMITY_SENSOR_TEST, not_supported_command_handler,
	 ARM11_PROCESSOR}
	,
	...
	,
#if defined(CONFIG_MACH_LGE_325_BOARD) || defined(CONFIG_MACH_LGE_IJB_BOARD_LGU) || defined (CONFIG_MACH_LGE_IJB_BOARD_SKT)
	{TEST_MODE_LTE_MODE_SELECTION, not_supported_command_handler,
	 ARM11_PROCESSOR}
	,
	...
	,
#endif
	{TEST_MODE_CHANGE_USB_DRIVER, not_supported_command_handler,
	 ARM11_PROCESSOR}
	,
	...
	,
	{TEST_MODE_SKIP_WELCOM_TEST, not_supported_command_handler,
	 ARM11_PROCESSOR}
	,
	{TEST_MODE_WIFI_MAC_RW, LGF_TestModeWiFiMACRW, ARM11_PROCESSOR}
	,
	/* 91 ~ */
	{TEST_MODE_DB_INTEGRITY_CHECK, LGF_TestModeDBIntegrityCheck,
	 ARM11_PROCESSOR}
	,
...
//                                             
	...
	,
	{TEST_MODE_WV_PROVISIONING_COMMAND, LGF_TestWVProvisioningCommand,
	 ARM11_PROCESSOR}
	,
			 ...
			 DIAG_TEST_MODE_F_rsp_type * pRsp)
{
	...
	if (diagpdev != NULL) {
		...
		case IDM_READ:
			...
			if (testmode_result[0] != 0xFF) {
				pRsp->ret_stat_code = TEST_OK_S;
				strncpy(pRsp->test_mode_rsp.idm_read,
					testmode_result, 16);
			} else {
				pRsp->ret_stat_code = TEST_FAIL_S;
				pr_info("%s: TEST_FAIL_S\n", __func__);
...
#define SEC_HW_MSB_MASK                0x007FE0FF
	...
	printk(KERN_ERR
	       "secure boot region : Address[0x%X] LSB[0x%X] : MSB[0x%X]  \n",
	       (unsigned int)MSM_QFPROM_BASE + 0x310,
	       secure_boot_region_value_lsb, secure_boot_region_value_msb);
	...
	printk(KERN_ERR
	       "oem config region : Address[0x%X] LSB[0x%X] MSB[0x%X]  \n",
	       (unsigned int)MSM_QFPROM_BASE + 0x220,
	       oem_config_region_value_lsb, oem_config_region_value_msb);

	read_permission_region_value_lsb =
	    secure_readl(MSM_QFPROM_BASE + 0x0A8);
	read_permission_region_value_msb =
	    secure_readl(MSM_QFPROM_BASE + 0x0AC);
	printk(KERN_ERR
	       "read permission region : Address[0x%X] LSB[0x%X] MSB[0x%X]  \n",
	       (unsigned int)MSM_QFPROM_BASE + 0x0A8,
	       read_permission_region_value_lsb,
	       read_permission_region_value_msb);

	write_permission_region_value_lsb =
	    secure_readl(MSM_QFPROM_BASE + 0x0B0);
	write_permission_region_value_msb =
	    secure_readl(MSM_QFPROM_BASE + 0x0B4);
	printk(KERN_ERR
	       "write permission region : Address[0x%X] LSB[0x%X] MSB[0x%X]  \n",
	       (unsigned int)MSM_QFPROM_BASE + 0x0B0,
	       write_permission_region_value_lsb,
	       write_permission_region_value_msb);
	...
	printk(KERN_ERR "spare region : Address[0x%X] LSB[0x%X] MSB[0x%X]  \n",
	       (unsigned int)MSM_QFPROM_BASE + 0x400, spare_region_value_lsb,
	       spare_region_value_msb);
#endif

	pRsp->ret_stat_code = TEST_FAIL_S;

	for (i = 0; i < 7; i++) {
		secondary_key_region_value_lsb =
		    secure_readl(MSM_QFPROM_BASE + 0x268 + (8 * i));
		secondary_key_region_value_msb =
		    secure_readl(MSM_QFPROM_BASE + 0x26C + (8 * i));
	...
	case OTP_WRITE:
		{
			...
			randI = 0;
			rand_cmd_buf.data =
			    (unsigned char *)virt_to_phys(&randI);
			rand_cmd_buf.size = 4;
			scm_result =
			    scm_call(254, 2, &rand_cmd_buf,
				     sizeof(rand_cmd_buf), NULL, 0);
			...
			scm_result =
			    scm_call(254, 3, &blow_cmd_buf,
				     sizeof(blow_cmd_buf), NULL, 0);
#else
			/* write permission check */
			if ((((write_permission_region_value_lsb &
			       blow_data_List[3].Row_LSB_Val) ==
			      blow_data_List[3].Row_LSB_Val)
			     &&
			     ((write_permission_region_value_msb &
			       blow_data_List[3].Row_MSB_Val) ==
			      blow_data_List[3].Row_MSB_Val))) {
			...
			for (i = 0; i < 7; i++) {
				...
				get_random_bytes
				    (&secondary_key_region_value_lsb, 4);
				get_random_bytes
				    (&secondary_key_region_value_msb, 4);
				...
				/* blow command */
				blow_cmd_buf.address =
				    (uint32) MSM_QFPROM_PHYS + 0x268 + (8 * i);
				blow_cmd_buf.LSB =
				    secondary_key_region_value_lsb &
				    SEC_HW_LSB_MASK;
				blow_cmd_buf.MSB =
				...
				printk(KERN_ERR
				       "generate secondary H/W key : LSB[0x%X] MSB[0x%X]  \n",
	...
	case OTP_READ:
		{
...
#ifdef QFUSETEST
			if (!
			    (((spare_region_value_lsb & blow_data_List[0].
			       Row_LSB_Val) == blow_data_List[0].Row_LSB_Val)
			     &&
			     ((spare_region_value_msb & blow_data_List[0].
			       Row_MSB_Val) ==
			      blow_data_List[0].Row_MSB_Val))) {
			...
			i = 0;
			if (!
			    (((secure_boot_region_value_lsb & blow_data_List[i].
			       Row_LSB_Val) == blow_data_List[i].Row_LSB_Val)
			     &&
			     ((secure_boot_region_value_msb & blow_data_List[i].
			       Row_MSB_Val) ==
			      blow_data_List[i].Row_MSB_Val))) {
				printk(KERN_ERR
				       " secure boot region not blow \n");
				break;
			}

			i = 1;
			if (!
			    (((oem_config_region_value_lsb & blow_data_List[i].
			       Row_LSB_Val) == blow_data_List[i].Row_LSB_Val)
			     &&
			     ((oem_config_region_value_msb & blow_data_List[i].
			       Row_MSB_Val) ==
			      blow_data_List[i].Row_MSB_Val))) {
				printk(KERN_ERR
				       " oem config region not blow \n");
				break;
			}
			...
			if (!
			    (((read_permission_region_value_lsb &
			     ...
			     ((read_permission_region_value_msb &
			      ...
			      blow_data_List[i].Row_MSB_Val))) {
				printk(KERN_ERR
				       "read permission region not blow \n");
				break;
			}
			...
			if (!
			    (((write_permission_region_value_lsb &
			     ...
			     ((write_permission_region_value_msb &
			      ...
			      blow_data_List[i].Row_MSB_Val))) {
				printk(KERN_ERR
				       " write permission region not blow \n");
				break;
			}
#endif
			pRsp->ret_stat_code = TEST_OK_S;
...
//                                                                        
	...
	case WV_ERASE:
		{
			...
			result =
			    call_usermodehelper("/system/bin/wvoemprovision",
						argv, (char **)envp,
						UMH_WAIT_PROC);
			command = "WV_ERASE";
		}
	...
	case WV_RINDEX:
		{
			...
			set_fs(KERNEL_DS);
			fd = sys_open((const char __user *)index_filepath,
				      O_RDONLY, 0);
			if (fd < 0) {
				set_fs(old_fs);
				// OK with index 0
				...
				printk(KERN_ERR
				       "WV_RINDEX : Not exist index file\n");
	...
	if (result == 0) {
		...
		set_fs(KERNEL_DS);
		fd = sys_open((const char __user *)result_filepath, O_RDONLY,
			      0);
		if (fd < 0) {
				    ...
				    DIAG_TEST_MODE_F_rsp_type * pRsp)
{
	...
	result =
	    call_usermodehelper("/system/bin/prapp", argv, (char **)envp,
				UMH_WAIT_PROC);
...
static ssize_t get_qfuse_blow_status(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	...
	i = 0;
	if (!
	    (((secure_boot_region_value_lsb & blow_data_List[i].Row_LSB_Val) ==
	      blow_data_List[i].Row_LSB_Val)
	     && ((secure_boot_region_value_msb & blow_data_List[i].Row_MSB_Val)
		 == blow_data_List[i].Row_MSB_Val))) {
	...
	i = 1;
	if (!
	    (((oem_config_region_value_lsb & blow_data_List[i].Row_LSB_Val) ==
	      blow_data_List[i].Row_LSB_Val)
	     && ((oem_config_region_value_msb & blow_data_List[i].Row_MSB_Val)
		 == blow_data_List[i].Row_MSB_Val))) {
	...
	i = 2;
	if (!
	    (((read_permission_region_value_lsb & blow_data_List[i].
	       Row_LSB_Val) == blow_data_List[i].Row_LSB_Val)
	     &&
	     ((read_permission_region_value_msb & blow_data_List[i].
	       Row_MSB_Val) == blow_data_List[i].Row_MSB_Val))) {
	...
	i = 3;
	if (!
	    (((write_permission_region_value_lsb & blow_data_List[i].
	       Row_LSB_Val) == blow_data_List[i].Row_LSB_Val)
	     &&
	     ((write_permission_region_value_msb & blow_data_List[i].
	       Row_MSB_Val) == blow_data_List[i].Row_MSB_Val))) {
...
static ssize_t set_qfuse_blow_status(struct device *dev,
