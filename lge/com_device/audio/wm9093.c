...
static void wm9093_codec_cmd(struct amp_data *data, wmCodecCmd * amp_codec_cmd)
{
	...
	err =
	    wm9093_amp_write_register(data, amp_codec_cmd->wmaddress,
				      amp_codec_cmd->wmdata);
...
#ifdef CONFIG_LGE_AUDIO_AMP_WM9093_CHECK_REGISTER_VALUE_BY_REREADING
	errRead =
	    wm9093_amp_read_register(data, amp_codec_cmd->wmaddress, &rData);
	...
	if (errRead > 0) {
		...
		pr_err
		    ("WM9093 I2C Read FAIL :errRead = %d[%X] reg = 0x%X, data = 0x%X\n",
		     errRead, errRead, amp_codec_cmd->wmaddress,
		     rData & 0xFFFF);
	}
#endif
}

static void wm9093_cmd_register_sequence(struct amp_data *data,
					 struct wm9093_CodecCmd_data *seq)
{
	int idx = 0;
...
static void wm9093_cmd_register_sequence_add_delay(int icodec_num,
...
#endif
			pr_info
			    ("************** wm9093 sleep in 50ms for reducing earphone popup noise *************\n");
...
static void wm9093_reg_dump(int icodec_num)
{
	...
	case ICODEC_HEADSET_ST_RX:
		...
		for (i = 0; i < data->pdata->hph_on.amp_function_size; i++) {
			read_result =
			    wm9093_amp_read_register(data,
						     data->pdata->hph_on.
						     amp_function[i].wmaddress,
						     &reg_val);
			if (read_result < 0) {
				...
				pr_info
				    ("WM9093[0x%02X] Oval: 0x%04X Tval: 0x%04X Readval: 0x%04X\n",
				     data->pdata->hph_on.amp_function[i].
				     wmaddress,
				     data->pdata->hph_on.amp_function[i].wmdata,
				     data->pdata->hph_on.amp_tuning_function[i].
				     wmdata, reg_val);
	...
	case ICODEC_SPEAKER_RX:
		...
		for (i = 0; i < data->pdata->speaker_on.amp_function_size; i++) {
			...
			if (read_result < 0) {
				...
				pr_info
				    ("WM9093[0x%02X] Oval: 0x%04X Tval: 0x%04X Readval: 0x%04X\n",
				     data->pdata->speaker_on.amp_function[i].
				     wmaddress,
				     data->pdata->speaker_on.amp_function[i].
				     wmdata,
				     data->pdata->speaker_on.
				     amp_tuning_function[i].wmdata, reg_val);
	...
	case ICODEC_HEADSET_ST_RX_SPEAKER_RX:	// simultaneously Ringing Headset and SPK
		...
		for (i = 0; i < data->pdata->hph_spk_on.amp_function_size; i++) {
			...
			if (read_result < 0) {
				...
				pr_info
				    ("WM9093[0x%02X] Oval: 0x%04X Tval: 0x%04X Readval: 0x%04X\n",
				     data->pdata->hph_spk_on.amp_function[i].
				     wmaddress,
				     data->pdata->hph_spk_on.amp_function[i].
				     wmdata,
				     data->pdata->hph_spk_on.
	...
	case ICODEC_HEADSET_ST_PLAYBACK_RX:
		...
		for (i = 0; i < data->pdata->hph_playback_on.amp_function_size;
		     i++) {
			read_result =
			    wm9093_amp_read_register(data,
						     data->pdata->
			...
			if (read_result < 0) {
				...
				pr_info
				    ("WM9093[0x%02X] Oval: 0x%04X Tval: 0x%04X Readval: 0x%04X\n",
				     data->pdata->hph_playback_on.
				     amp_function[i].wmaddress,
				     data->pdata->hph_playback_on.
				     amp_function[i].wmdata,
				     data->pdata->hph_playback_on.
				     amp_tuning_function[i].wmdata, reg_val);
	...
	case ICODEC_SPEAKER_PLAYBACK_RX:
		...
		for (i = 0;
		     i < data->pdata->speaker_playback_on.amp_function_size;
		     i++) {
			read_result =
			    wm9093_amp_read_register(data,
						     data->pdata->
			...
			if (read_result < 0) {
				...
				pr_info
				    ("WM9093[0x%02X] Oval: 0x%04X Tval: 0x%04X Readval: 0x%04X\n",
				     data->pdata->speaker_playback_on.
				     amp_function[i].wmaddress,
				     data->pdata->speaker_playback_on.
				     amp_function[i].wmdata,
				     data->pdata->speaker_playback_on.
	...
	default:
		for (i = 0; i < 0x50; i++) {
			read_result =
			    wm9093_amp_read_register(data, i, &reg_val);
			if (read_result < 0) {
				pr_info("failed to read codec register\n");
				break;
			} else
...
static void wm9093_set_amp_path(int icodec_num)
{
	...
	if ((icodec_num == ICODEC_HEADSET_ST_RX)
	    || (icodec_num == ICODEC_HEADSET_ST_RX_SPEAKER_RX)
	    || (icodec_num == ICODEC_HEADSET_ST_PLAYBACK_RX)) {
		if (icodec_num == ICODEC_HEADSET_ST_RX_SPEAKER_RX) {
			pr_info(KERN_INFO
				"headset_speaker_pre setting for elimiating AMP pop noise");
			wm9093_cmd_register_sequence(data,
						     &(data->pdata->
						       headset_speaker_pre));
		} else {
			pr_info(KERN_INFO
				"headset_pre setting for elimiating AMP pop noise");
			wm9093_cmd_register_sequence(data,
						     &(data->pdata->
						       headset_pre));
		}
...
#if defined(CONFIG_MACH_LGE_325_BOARD_DCM) || defined(CONFIG_MACH_LGE_325_BOARD_VZW)
		wm9093_cmd_register_sequence_add_delay(ICODEC_HEADSET_ST_RX,
						       data,
						       &(data->pdata->hph_on));
#else
...
#if defined(CONFIG_MACH_LGE_325_BOARD_DCM) || defined(CONFIG_MACH_LGE_325_BOARD_VZW)
		wm9093_cmd_register_sequence_add_delay(ICODEC_SPEAKER_RX, data,
						       &(data->pdata->
							 speaker_on));
#else
...
//                                                  
	case ICODEC_HEADSET_ST_RX_SPEAKER_RX:	// simultaneously Ringing Headset and SPK
		pr_info("AMP ON: voc_codec %d for HEADSET_ST_RX_SPEAKER_RX\n",
			icodec_num);
#if defined(CONFIG_MACH_LGE_325_BOARD_DCM) || defined(CONFIG_MACH_LGE_325_BOARD_VZW)
		wm9093_cmd_register_sequence_add_delay
		    (ICODEC_HEADSET_ST_RX_SPEAKER_RX, data,
		     &(data->pdata->hph_spk_on));
#else
...
#if defined(CONFIG_MACH_LGE_325_BOARD_DCM) || defined(CONFIG_MACH_LGE_325_BOARD_VZW)
		wm9093_cmd_register_sequence_add_delay(ICODEC_TTY_RX, data,
						       &(data->pdata->tty_on));
#else
...
#if defined(CONFIG_MACH_LGE_325_BOARD_DCM) || defined(CONFIG_MACH_LGE_325_BOARD_VZW)
		wm9093_cmd_register_sequence_add_delay
		    (ICODEC_SPEAKER_PLAYBACK_RX, data,
		     &(data->pdata->speaker_playback_on));
#else
		wm9093_cmd_register_sequence(data,
					     &(data->pdata->
	...
	case ICODEC_SPEAKER_PLAYBACK_VZWNAVI:	// Playback not call
		pr_info
		    ("AMP ON: voc_codec %d for ICODEC_SPEAKER_VZW_NAVI amp\n",
		     icodec_num);
#if defined(CONFIG_MACH_LGE_325_BOARD_DCM) || defined(CONFIG_MACH_LGE_325_BOARD_VZW)
		wm9093_cmd_register_sequence_add_delay
		    (ICODEC_SPEAKER_PLAYBACK_VZWNAVI, data,
		     &(data->pdata->speaker_vzwnavi_on));
#else
		wm9093_cmd_register_sequence(data,
					     &(data->pdata->
					       speaker_vzwnavi_on));
...
#if defined(CONFIG_MACH_LGE_325_BOARD_DCM) || defined(CONFIG_MACH_LGE_325_BOARD_VZW)
		wm9093_cmd_register_sequence_add_delay
		    (ICODEC_HEADSET_ST_PLAYBACK_RX, data,
		     &(data->pdata->hph_playback_on));
#else
		wm9093_cmd_register_sequence(data,
					     &(data->pdata->hph_playback_on));
...
static int wm9093_amp_ctl_probe(struct i2c_client *client,
