...
//#define CONFIG_MSM_CAMERA_DEBUG_IMX105
...
static struct msm_sensor_output_info_t imx105_dimensions[] = {
	...
	{			//MSM_SENSOR_RES_FULL
	 ...
	 .vt_pixel_clk = 134400000,	// 134400000, // mo2jonghoo.lee 2013.02.01 330000000, //174000000,
	 .op_pixel_clk = 134400000,	//134400000, // mo2jonghoo.lee 2013.02.01 69600000,
	 .binning_factor = 1,
	 },
	{			//MSM_SENSOR_RES_QTR
	 ...
	 .vt_pixel_clk = 134400000,	//67200000, //174000000,
	 .op_pixel_clk = 67200000,	//134400000, // mo2jonghoo.lee 2013.02.01 69600000,
/*
...
*/
	...
	{			//MSM_SENSOR_RES_FHD // MSM_SENSOR_RES_2
	 ...
	 .vt_pixel_clk = 134400000,	//134400000, //174000000,
	 .op_pixel_clk = 134400000,	//134400000, // mo2jonghoo.lee 2013.02.01 69600000,
	 .binning_factor = 1,
	 },
};
...
//                                                                  
static int16_t imx105_i2c_write_b_af(struct msm_camera_i2c_client
				     *sensor_i2c_client, uint8_t baddr,
				     uint8_t bdata)
{
	int32_t rc;
	unsigned char buf[2];
	...
	buf[0] = baddr;
	buf[1] = bdata;
	rc = msm_camera_i2c_txdata_manual(sensor_i2c_client->client->adapter,
					  IMX105_AF_SLAVE_ADDR, buf, 2);
	if (rc < 0)
...
//                                                                  
static int32_t imx105_i2c_read_w_eeprom(struct msm_camera_i2c_client
					...
					uint16_t * rdata)
{
	...
	rc = msm_camera_i2c_rxdata_manual(sensor_i2c_client->client->adapter,
					  IMX105_EEPROM_SLAVE_ADDR << 1, buf,
					  1);
	if (rc < 0) {
		printk(KERN_EMERG
		       "[CAMERA]1 :imx105_i2c_read_eeprom 0x%x failed!\n",
		       reg_addr);
		return rc;
	}
	*rdata = buf[0];
	...
	if (rc < 0) {
		printk(KERN_EMERG
		       "[CAMERA]2 :imx105_i2c_read_eeprom 0x%x failed!\n",
		       reg_addr);
		return rc;
	}
	*rdata = (*rdata << 8) | buf[0];
...
static int imx105_read_eeprom_data(struct msm_sensor_ctrl_t *s_ctrl,
				   struct sensor_cfg_data *cfg)
{
	...
	addr = 0x0000;
	rc = imx105_i2c_read_w_eeprom(s_ctrl->sensor_i2c_client, addr,
				      &eepromdata);
	if (rc < 0) {
	...
	cfg->cfg.calib_info.macro_2_inf =
	    cfg->cfg.calib_info.stroke_amt - cfg->cfg.calib_info.af_pos_inf;
	...
	fastaf_infinitymargin =
	    (uint16_t) (cfg->cfg.calib_info.af_pos_inf / 10);
	fastaf_macromargin = fastaf_infinitymargin;
	CDBG("fastaf_infinitymargin = %d\n", fastaf_infinitymargin);
	fastaf_stepsize =
	    (uint16_t) ((cfg->cfg.calib_info.macro_2_inf +
			 fastaf_infinitymargin +
			 fastaf_macromargin) /
			(IMX105_TOTAL_STEPS_NEAR_TO_FAR));
	imx105_step_position_table[0] =
	    cfg->cfg.calib_info.af_pos_inf - fastaf_infinitymargin;
	...
	af_optical_infinity_value =
	    (uint16_t) cfg->cfg.calib_info.af_pos_inf -
	    (uint16_t) fastaf_infinitymargin;
...
//      s_ctrl->pict_fps_divider = fps->pict_fps_div;  // john.park 2013.02.27  compile error fix
	...
	if (s_ctrl->curr_res == MSM_SENSOR_RES_QTR)
		...
		total_lines_per_frame = (uint16_t) (((IMX105_FULL_SIZE_HEIGHT + IMX105_VER_FULL_BLK_LINES) * s_ctrl->fps_divider) / 0x400);	/* s_ctrl->pict_fps_divider) / 0x400 */// john.park 2013.02.27  compile error fix
...
//      rc = imx105_i2c_write_b_sensor(REG_GROUPED_PARAMETER_HOLD, GROUPED_PARAMETER_HOLD);
	...
	CDBG("imx105 setting REG_ANALOGUE_GAIN_CODE_GLOBAL_HI = 0x%X\n",
	     gain_msb);
	rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
				  REG_ANALOGUE_GAIN_CODE_GLOBAL_HI, gain_msb,
				  MSM_CAMERA_I2C_BYTE_DATA);
//      rc = imx105_i2c_write_b_sensor(REG_ANALOGUE_GAIN_CODE_GLOBAL_HI, gain_msb);
	...
	rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
				  REG_ANALOGUE_GAIN_CODE_GLOBAL_LO, gain_lsb,
				  MSM_CAMERA_I2C_BYTE_DATA);
...
//      rc = imx105_i2c_write_b_sensor(REG_FRAME_LENGTH_LINES_LO, frame_length_line_lsb);
	...
	rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
				  REG_COARSE_INTEGRATION_TIME_HI, intg_time_msb,
				  MSM_CAMERA_I2C_BYTE_DATA);
//      rc = imx105_i2c_write_b_sensor(REG_COARSE_INTEGRATION_TIME_HI,intg_time_msb);
	...
	rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
				  REG_COARSE_INTEGRATION_TIME_LO, intg_time_lsb,
				  MSM_CAMERA_I2C_BYTE_DATA);
				 ...
				 int direction, int32_t num_steps)
{
	...
	if (imx105_ctrl->curr_lens_pos != dest_lens_position) {
		...
		rc = imx105_i2c_write_b_af(s_ctrl->sensor_i2c_client,
					   codeval_msb, codeval_lsb);
		if (rc < 0) {
...
//                                                                
static int32_t imx105_actuator_move_focus(struct msm_actuator_ctrl_t *a_ctrl,
					  ...
					  *move_params)
{
	...
	CDBG("%s index =[%d] = value = %d, curr_lens_pos = %d\n",
	     __func__, dest_step_position, dest_lens_position,
	     imx105_ctrl->curr_lens_pos);
	if (imx105_ctrl->curr_lens_pos != dest_lens_position) {
		codeval_msb = (AF_ISRC_MODE_ADDR |
			       ((dest_lens_position & 0x0300) >> 8));
		codeval_lsb = dest_lens_position & 0x00FF;
		rc = imx105_i2c_write_b_af(&a_ctrl->i2c_client, codeval_msb,
					   codeval_lsb);
		if (rc < 0) {
			CDBG("imx105 I2C Failed line %d\n", __LINE__);
			return rc;
...
static int32_t imx105_set_default_focus(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	CDBG("%s ==  enter\n", __func__);
	rc = imx105_i2c_write_b_af(s_ctrl->sensor_i2c_client,
				   AF_DIRECT_MODE_ADDR, AF_MECH_INFINITY);
	if (rc < 0)
		return rc;
	usleep(50);
...
//                                                                
static int32_t imx105_actuator_set_default_focus(struct msm_actuator_ctrl_t
						 *a_ctrl,
						 struct
...
struct msm_actuator msm_actuator_table_imx105 = {
	...
	.func_tbl = {
		     ...
		     imx105_actuator_set_default_focus,
...
static int16_t imx105_init_focus(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	rc = imx105_i2c_write_b_af(s_ctrl->sensor_i2c_client,
				   AF_SRSS_RFS_VALUE_ADDR, AF_SRSS_RFS_VAL);
	if (rc < 0) {
		pr_err("[JJONG]imx105_init_focus af i2c failed 1\n");
		return rc;
	}
	rc = imx105_i2c_write_b_af(s_ctrl->sensor_i2c_client,
				   AF_SSTS_RS_VALUE_ADDR, AF_SSTS_RS_VAL);
	if (rc < 0) {
...
static int32_t imx105_config(struct msm_sensor_ctrl_t *s_ctrl,
			     void __user * argp)
{
...
//                                                                 
	...
	case CFG_GET_PREV_L_PF:
		rc = imx105_get_prev_lines_pf(s_ctrl, &cdata.cfg.prevl_pf);
	...
	case CFG_GET_PREV_P_PL:
		rc = imx105_get_prev_pixels_pl(s_ctrl, &cdata.cfg.prevp_pl);
	...
	case CFG_GET_PICT_L_PF:
		rc = imx105_get_pict_lines_pf(s_ctrl, &cdata.cfg.pictl_pf);
	...
	case CFG_GET_PICT_P_PL:
		rc = imx105_get_pict_pixels_pl(s_ctrl, &cdata.cfg.pictp_pl);
	...
	case CFG_GET_AF_MAX_STEPS:
		rc = imx105_get_af_max_steps(s_ctrl, &cdata.max_steps);
