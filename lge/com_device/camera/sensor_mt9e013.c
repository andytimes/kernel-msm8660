				     ...
				     uint8_t * rxdata, int length)
{
	...
	if (i2c_transfer(mt9e013_client->adapter, msgs, 2) < 0) {
		printk(KERN_EMERG
		       "[CAMERA] %s: mt9e013_i2c_rxdata faild 0x%x\n", __func__,
		       saddr);
		return -EIO;
	}
	return 0;
...
static int32_t mt9e013_i2c_write_w_sensor(unsigned short waddr, uint16_t wdata)
{
	...
	if (rc < 0) {
		printk(KERN_EMERG
		       "i2c_write_b failed, addr = 0x%x, val = 0x%x!\n", waddr,
		       wdata);
	}
	return rc;
}
...
//                                                 
static int32_t mt9e013_i2c_read_w_eeprom(uint16_t raddr, uint16_t * rdata)
{
	int32_t rc = 0;
	unsigned char buf;
...
static int32_t mt9e013_read_5100k_data(void)
{
	...
	k = 0;
	lsc_data =
	    kzalloc(sizeof(struct mt9e013_i2c_reg_conf) * 106, GFP_KERNEL);
	...
	for (j = 0; j < 5; j++) {
		for (i = 0; i < data_size; i++) {
			...
			if (rc < 0) {
				printk(KERN_EMERG
				       "[CAMERA]%s : Fail to read!! j=%d,i=%d Source line number : %d\n",
...
static int32_t mt9e013_read_awb_data(struct sensor_cfg_data *cfg, bool bresult)
{
	...
	printk(KERN_EMERG "[CAMERA]%s : line:%d : GR/GB 0x%04x\n", __func__,
	       __LINE__, cfg->cfg.calib_info.gr_over_gb);
...
static void mt9e013_get_pict_fps(uint16_t fps, uint16_t * pfps)
{
	...
	d1 = mt9e013_regs.reg_prev[E013_FRAME_LENGTH_LINES].wdata
	    * 0x00000400 / mt9e013_regs.reg_snap[E013_FRAME_LENGTH_LINES].wdata;
	d2 = mt9e013_regs.reg_prev[E013_LINE_LENGTH_PCK].wdata
	    * 0x00000400 / mt9e013_regs.reg_snap[E013_LINE_LENGTH_PCK].wdata;
	divider = d1 * d2 / 0x400;
...
static int32_t mt9e013_set_fps(struct fps_cfg *fps)
{
	...
	if (mt9e013_ctrl->curr_res == QTR_SIZE)
		...
		total_lines_per_frame =
		    mt9e013_regs.reg_FHD[E013_FRAME_LENGTH_LINES].wdata;
	else
		total_lines_per_frame =
	...
	if (mt9e013_ctrl->curr_res == FULL_SIZE) {
		total_lines_per_frame = (uint16_t)
		    (total_lines_per_frame * mt9e013_ctrl->pict_fps_divider /
		     0x400);
	} else {
		total_lines_per_frame = (uint16_t)
...
static int32_t mt9e013_write_exp_gain(uint16_t gain, uint32_t line)
{
	...
	if (mt9e013_ctrl->curr_res == QTR_SIZE) {
		frame_length_lines =
		    mt9e013_regs.reg_prev[E013_FRAME_LENGTH_LINES].wdata;
		frame_length_lines =
		    frame_length_lines * mt9e013_ctrl->fps_divider / 0x400;
		line =
		    (uint32_t) (line * mt9e013_ctrl->fps_divider / 0x00000400);
	} else if (mt9e013_ctrl->curr_res == FHD_SIZE) {
		frame_length_lines =
		    mt9e013_regs.reg_FHD[E013_FRAME_LENGTH_LINES].wdata;
		frame_length_lines =
	...
	} else			//snapshot
	{
		frame_length_lines =
		    mt9e013_regs.reg_snap[E013_FRAME_LENGTH_LINES].wdata;
		frame_length_lines =
		    frame_length_lines * mt9e013_ctrl->pict_fps_divider / 0x400;
		mt9e013_ctrl->pict_fps_divider = mt9e013_ctrl->fps_divider * 2;
		line =
		    (uint32_t) (line * mt9e013_ctrl->pict_fps_divider /
				0x00000400);
	}
#else
	...
	CDBG("[CAMERA][1] mt9e013_write_exp_gain : frame_length_lines = NONE, mt9e013_ctrl->fps_divider = %d, mt9e013_ctrl->pict_fps_divider = %d\n",
	     /*frame_length_lines, */ mt9e013_ctrl->fps_divider,
	     mt9e013_ctrl->pict_fps_divider);
...
//      rc = mt9e013_i2c_write_w_sensor(REG_FRAME_LENGTH_LINES, frame_length_lines);
...
#if 0				// test 1012
	{
		...
		printk(KERN_EMERG
		       "[CAMERA][1012] mt9e013_write_exp_gain : line=%d gain=%d REG_GLOBAL_GAIN=%d REG_COARSE_INTEGRATION_TIME=%d REG_LENS_SHADING=0x%04x\n",
...
static int32_t mt9e013_move_focus(int direction, int32_t num_steps)
{
	...
	mt9e013_i2c_write_w_sensor(REG_VCM_STEP_TIME, mt9e013_vcm_step_time);
...
static int32_t mt9e013_sensor_setting(int update_type, int rt)
{
	...
	printk(KERN_EMERG
	       "[CAMERA]mt9e013_sensor_setting :::: update_type = %d, rt = %d\n",
	       update_type, rt);
	mt9e013_stop_stream();
	msleep(5);		//                                                       
	if (update_type == REG_INIT) {
		printk(KERN_EMERG
		       "[CAMERA]mt9e013_sensor_setting :::: REG_INIT\n");
		...
		if (rt == QTR_SIZE) {
			printk(KERN_EMERG
			       "[CAMERA]mt9e013_sensor_setting :::: QTR_SIZE\n");
			//                                                                
			//                                                                                                                       
...
#if 1
			{
				total_lines_per_frame =
				    mt9e013_regs.
				    reg_prev[E013_FRAME_LENGTH_LINES].wdata;
				mt9e013_group_hold_on();
			...
			printk(KERN_EMERG
			       "[CAMERA]mt9e013_sensor_setting :::: FHD_SIZE\n");
			//                                                                
			//                                                                                                                       
			...
			//msleep(1);
			//                                                         
			mt9e013_i2c_write_w_table(mt9e013_regs.reg_FHD,
						  mt9e013_regs.reg_FHD_size);
			//                                                                
#if 1
			{
				total_lines_per_frame =
				    mt9e013_regs.
				    reg_FHD[E013_FRAME_LENGTH_LINES].wdata;
				mt9e013_group_hold_on();
				rc = mt9e013_i2c_write_w_sensor
				    (REG_FRAME_LENGTH_LINES,
				     total_lines_per_frame);
			...
			printk(KERN_EMERG
			       "[CAMERA]mt9e013_sensor_setting :::: FULL_SIZE\n");
			//                                                         
			//mt9e013_i2c_write_w_table(mt9e013_regs.reg_pll,       mt9e013_regs.reg_pll_size);
			//msleep(1);
			//                                                         
...
static int32_t mt9e013_video_config(int mode)
{
	...
	printk(KERN_EMERG
	       "[CAMERA]mt9e013_video_config :::: mode = %d, prev_res = %d\n",
	       mode, mt9e013_ctrl->prev_res);
	/* change sensor resolution if needed */
	if (mt9e013_sensor_setting(UPDATE_PERIODIC, mt9e013_ctrl->prev_res) < 0)
		return rc;
...
static int32_t mt9e013_set_sensor_mode(int mode, int res)
{
	int32_t rc = 0;

	printk(KERN_EMERG
	       "[CAMERA][KERNEL]mt9e013_set_sensor_mode ::: mode = %d, res = %d\n",
	       mode, res);
...
static int mt9e013_read_eeprom_data(struct sensor_cfg_data *cfg)
{
	...
	CDBG("[CAMERA]%s:%d: AWB: r/g:0x%04x b/g:0x%04x gr/gb:0x%04x\n",
	     ...
	     cfg->cfg.calib_info.b_over_g, cfg->cfg.calib_info.gr_over_gb);
...
static int mt9e013_probe_init_sensor(const struct msm_camera_sensor_info *data)
{
	...
	if (chipid != 0x4B00) {
		rc = -ENODEV;
		printk(KERN_EMERG
		       "[CAMERA]mt9e013_probe_init_sensor fail chip id doesnot match\n");
...
int mt9e013_sensor_open_init(const struct msm_camera_sensor_info *data)
{
	...
	if (rc < 0) {
		printk(KERN_EMERG
		       "[CAMERA]Calling mt9e013_sensor_open_init fail1\n");
		return rc;
	}
	CDBG("[CAMERA]%s: %d\n", __func__, __LINE__);
#if 1
				...
				struct msm_sensor_ctrl *s)
{
	...
	if (rc < 0 || mt9e013_client == NULL) {
		rc = -ENOTSUPP;
		printk(KERN_EMERG
		       "[CAMERA] I2C add driver failed****** rc : %d\n", rc);
		goto probe_fail;
	}
//                                          
//      msm_camio_clk_rate_set(MT9E013_MASTER_CLK_RATE);
