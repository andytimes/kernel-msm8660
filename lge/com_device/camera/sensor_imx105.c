...
#define	IMX105_VER_QTR_BLK_LINES	58	//                                                                       
					   ...
					   *reg_conf_tbl, int num) 
{
		    ...
		    imx105_i2c_write_b_sensor(reg_conf_tbl->waddr,
...
 static int16_t imx105_i2c_write_b_af(uint8_t baddr,  uint8_t bdata) 
{
		    ...
		    ("afi2c_write failed, saddr = 0x%x addr = 0x%x, val =0x%x!",
...
 static int32_t imx105_set_fps(struct fps_cfg *fps) 
{
		    ...
		    (uint16_t) (((IMX105_QTR_SIZE_HEIGHT +
				  IMX105_VER_QTR_BLK_LINES) *
				 imx105_ctrl->fps_divider) / 0x400);
	
	else if (imx105_ctrl->curr_res == FHD_SIZE)	//shchang@qualcomm.com for FHD fixed FPS
		total_lines_per_frame =
		    (uint16_t) (((IMX105_FHD_SIZE_HEIGHT +
				  IMX105_VER_FHD_BLK_LINES) *
				 imx105_ctrl->fps_divider) / 0x400);
	
	else
		total_lines_per_frame =
		    (uint16_t) (((IMX105_FULL_SIZE_HEIGHT +
				  IMX105_VER_FULL_BLK_LINES) *
				 imx105_ctrl->pict_fps_divider) / 0x400);
	 rc =
	    imx105_i2c_write_b_sensor(REG_FRAME_LENGTH_LINES_HI,
				      ((total_lines_per_frame & 0xFF00) >> 8));
	 rc =
	    imx105_i2c_write_b_sensor(REG_FRAME_LENGTH_LINES_LO,
...
static int32_t imx105_write_NoiseReduction(uint16_t gain) 
{
	    ...
	    imx105_i2c_write_b_sensor(REG_GROUPED_PARAMETER_HOLD,
	...
	rc = imx105_i2c_write_b_sensor(REG_NR_MFILTER_LEVEL, 0x1E);
		...
		rc = imx105_i2c_write_b_sensor(REG_NR_MFILTER_LEVEL, 0x1E);
		    ...
		    ("Func : NoiseReduction - Writing bright2 light NR Value..\n");
		...
		rc = imx105_i2c_write_b_sensor(REG_NR_MFILTER_LEVEL, 0x1E);
		    ...
		    ("Func : NoiseReduction - Writing bright1 light NR Value..\n");
...
//                                                                     
#ifdef SHUTTER_LAG_TEST
static int32_t imx105_set_shutter_time(int reg_set, uint16_t gain,
				       uint32_t line) 
{
		    ...
		    rc =
		    imx105_i2c_write_b_sensor(REG_COARSE_INTEGRATION_TIME_HI,
					      intg_time_msb);
		rc =
		    imx105_i2c_write_b_sensor(REG_COARSE_INTEGRATION_TIME_LO,
					      intg_time_lsb);
		 
		    // register 2 set that separate 1080p preview and norma preview for CTS Test.
		    if (imx105_ctrl->prev_res == FHD_SIZE)
			 {
			 CDBG
			    ("____  SHUTTER_REGSET PREVIEW MODE IS 1080P _____\n");
			    ...
			    imx105_i2c_write_w_table(imx105_regs.
						     ...
						     shutter2_1080ptbl_size);
			 }
		
		else
			 {
			CDBG
			    ("____  SHUTTER_REGSET PREVIEW MODE IS PREVIEW _____\n");
...
//                                                     
	    ...
	    (uint8_t) ((frame_length_lines & 0xFF00) >> 8);
	    ...
	    imx105_i2c_write_b_sensor(REG_ANALOGUE_GAIN_CODE_GLOBAL_HI,
	    ...
	    imx105_i2c_write_b_sensor(REG_ANALOGUE_GAIN_CODE_GLOBAL_LO,
...
//                                                                  
		    CDBG
		    ("imx105 setting REG_COARSE_INTEGRATION_TIME_HI = 0x%X\n",
		    ...
		    ("imx105 setting REG_COARSE_INTEGRATION_TIME_LO = 0x%X\n",
...
 static int32_t imx105_set_pict_exp_gain(uint16_t gain, uint32_t line) 
{
	    ...
	    if (NR_ENABLE)
			...
			CDBG("Writing IMX105 Noise reduction is failed...\n");
...
  static int16_t imx105_af_init(void) 
{
	...
	unsigned short vcmcodeinf, vcmcode10cm, vcmcode100cm, vcmcodesc,
	    vcmcodeos;
...
static int32_t imx105_move_focus(int direction, int32_t num_steps) 
{
	    ...
	    (step_direction * num_steps);
		    ...
		    ("imx105_move_focus ==  imx105_ctrl->curr_step_pos:exit\n");
...
//                                                       
			    ...
			    CDBG
			    ("Func : Init - Writing bright1 light NR Value..\n");
			 rc =
			    imx105_i2c_write_b_sensor(REG_2DNR_ENABLE, 0x79);
			rc =
			    imx105_i2c_write_b_sensor(REG_2DNR_GLINKINGSWITCH,
						      0x0E);
			rc = imx105_i2c_write_b_sensor(REG_NR_STRENGTH, 0x0F);	//15
			rc = imx105_i2c_write_b_sensor(REG_NR_MFILTER_LEVEL,
						       0x1E);
			    ...
			    imx105_i2c_write_w_table(imx105_regs.init_tbl,
...
//                                                                  
			    ...
			    if (config_csi == 0) {
				    ...
				    msm_camio_csi_config(&imx105_csi_params);
...
  static int imx105_probe_init_done(const struct msm_camera_sensor_info
				       *data) 
{
...
//                                                     
 static int imx105_probe_init_sensor(const struct msm_camera_sensor_info
				       *data) 
{
...
//                                                                                                                                                   
	    if (rc < 0) {
			...
			__func__);
			    ...
			    ("%s :: (failed ) af i2c write fail during imx105_af_retry_init() \n",
			     __func__);
...
//              goto init_fail;
		goto init_done;
		    ...
		    ("%s :: (success)eeprom read success during af_init() \n",
		     __func__);
...
 static int cam_debug_init(void) 
{
	       ...
	       ("af_codeperstep", S_IRUGO | S_IWUSR, cam_dir, NULL,
		&af_codeperstep))
		return -ENOMEM;
	if (!debugfs_create_file
	     ("af_linear", S_IRUGO | S_IWUSR, cam_dir, NULL, &af_linear))
		return -ENOMEM;
	if (!debugfs_create_file
	     ("af_step", S_IRUGO | S_IWUSR, cam_dir, NULL, &af_step))
