...
static int mt9m114_reset(const struct msm_camera_sensor_info *dev)
{
	...
	if (!rc) {
		...
		CDBG("%s: reset 0 = %d, rc = %d/n", __func__, dev->sensor_reset,
		     rc);
		mdelay(50);
		rc = gpio_direction_output(dev->sensor_reset, 1);
		CDBG("%s: reset 1 = %d, rc = %d/n", __func__, dev->sensor_reset,
		     rc);
	} else {
...
static int32_t mt9m114_i2c_write_w_sensor(unsigned short waddr,
...
static int32_t mt9m114_i2c_write_table(struct mt9m114_i2c_reg_conf const
	...
	for (i = 0; i < num_of_items_in_table; i++) {
		if (reg_conf_tbl->waddr == 0xFFFF) {
			...
			for (i = 0; i < 50; i++) {	// max delay ==> 500 ms 
				...
				CDBG_VT
				    ("### %s :  Polling set, 0x0080 Reg : 0x%x\n",
				     __func__, test_data);
			}
		} else if (reg_conf_tbl->waddr == 0x301A) {
			unsigned short test_data = 0;
			rc = mt9m114_i2c_read(mt9m114_client->addr, 0x301A,
					      &test_data, WORD_LEN);
			if (rc < 0)
				return rc;

			rc = mt9m114_i2c_write_w_sensor(0x301A,
							test_data | 0x0200);
			if (rc < 0)
				return rc;
		...
		} else if ((reg_conf_tbl->waddr == 0x0080)
			   && ((reg_conf_tbl->wdata == 0x8000)
			       || (reg_conf_tbl->wdata == 0x0001))) {
			unsigned short test_data = 0;
			rc = mt9m114_i2c_read(mt9m114_client->addr, 0x0080,
					      &test_data, WORD_LEN);
			if (rc < 0)
				return rc;
		...
		for (loop = 0; loop < ptr_list->num_regs; loop++) {
			if (ptr_list->list_regs[loop].mem.addr == 0xFFFF) {
				...
				CDBG("[icebox]%s:msleep addr = %d, val = %d\n",
				     __func__,
				     ptr_list->list_regs[loop].mem.addr,
				     ptr_list->list_regs[loop].mem.val);
				rc = 0;
			} else
				...
				rc = mt9m114_i2c_write_w_sensor(ptr_list->
								list_regs[loop].
								mem.addr,
								ptr_list->
								list_regs[loop].
...
static int mt9m114_set_effect(int effect)
{
	...
	case CAMERA_EFFECT_NEGATIVE:
		rc = mt9m114_i2c_write_table(mt9m114_regs.effect_negative_tbl,
					     mt9m114_regs.
					     effect_negative_tbl_size);
		if (rc < 0)
			return rc;
		break;
	case CAMERA_EFFECT_SOLARIZE:
		rc = mt9m114_i2c_write_table(mt9m114_regs.
					     effect_solarization_tbl,
					     mt9m114_regs.
					     effect_solarization_tbl_size);
		if (rc < 0)
			return rc;
		break;
...
static int mt9m114_set_iso(int mode)
{
	...
	case CAMERA_ISO_AUTO:
		rc = mt9m114_i2c_write_table(mt9m114_regs.iso_default_tbl,
					     mt9m114_regs.iso_default_tbl_size);
		if (rc < 0)
			return rc;
		break;
	case CAMERA_ISO_100:
		rc = mt9m114_i2c_write_table(mt9m114_regs.iso_100_tbl,
					     mt9m114_regs.iso_100_tbl_size);
		if (rc < 0)
			return rc;
		break;
	case CAMERA_ISO_200:
		rc = mt9m114_i2c_write_table(mt9m114_regs.iso_200_tbl,
					     mt9m114_regs.iso_200_tbl_size);
		if (rc < 0)
			return rc;
		break;
	case CAMERA_ISO_400:
		rc = mt9m114_i2c_write_table(mt9m114_regs.iso_400_tbl,
					     mt9m114_regs.iso_400_tbl_size);
		if (rc < 0)
			return rc;
		break;
	default:
	...
	rc = mt9m114_i2c_write_table(mt9m114_regs.change_config_tbl,
				     mt9m114_regs.change_config_tbl_size);
	if (rc < 0)
		return rc;
...
static int mt9m114_reg_preview(void)
{
	...
	rc = mt9m114_i2c_write_table(mt9m114_regs.prev_tbl,
				     mt9m114_regs.prevtbl_size);
	if (rc < 0)
		return rc;
...
static int mt9m114_reg_snapshot(void)
{
	...
	rc = mt9m114_i2c_write_table(mt9m114_regs.snap_tbl,
				     mt9m114_regs.snaptbl_size);
	if (rc < 0)
		return rc;
...
static long mt9m114_set_sensor_mode(int mode)
{
	...
	temp_rc =
	    mt9m114_i2c_read(mt9m114_client->addr, 0xDC01, &test_data,
			     WORD_LEN);
	if (temp_rc < 0)
...
//                                                                                
		...
		mdelay(5);
		rc = mt9m114_i2c_read(mt9m114_client->addr, 0x0990, &test_data,
				      WORD_LEN);
		if (rc < 0)
			return rc;
		...
		if (test_data == 0x3100) {
			CDBG_VT
			    ("### %s: Skip preview mode , 0xDC01 Reg == 0x3100 \n",
			     __func__);
			return 0;
		} else {
			CDBG_VT
			    ("### %s: Going preview mode ,0xDC01 Reg -> 0x%x\n",
			     __func__, test_data);
		}
//                                                                                
...
//                                                 
	...
	if (prev_brightness_mode != 6)	// Set again when mode is not center 
	{
		...
		rc = mt9m114_i2c_write_w_sensor(0x337E,
						brightness_table
						[prev_brightness_mode]);
		if (rc < 0)
			return rc;

		rc = mt9m114_i2c_write_w_sensor(0xC940,
						gamma_table_sub
						[prev_brightness_mode]);
		if (rc < 0)
			return rc;
