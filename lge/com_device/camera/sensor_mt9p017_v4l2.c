...
static uint16_t mt9p017_step_position_table[MT9P017_TOTAL_STEPS_NEAR_TO_FAR +
					    1];
//                                                 
...
static struct msm_camera_i2c_conf_array mt9p017_init_conf[] = {
	{&mipi_settings[0], ARRAY_SIZE(mipi_settings), 0,
	 MSM_CAMERA_I2C_WORD_DATA},
	{&pll_settings[0], ARRAY_SIZE(pll_settings), 0,
	 MSM_CAMERA_I2C_WORD_DATA},
	{&recommend_settings[0], ARRAY_SIZE(recommend_settings), 0,
	 MSM_CAMERA_I2C_WORD_DATA},
	{&lensrolloff_tbl[0], ARRAY_SIZE(lensrolloff_tbl), 0,
	 MSM_CAMERA_I2C_WORD_DATA},
	{&nr_settings[0], ARRAY_SIZE(nr_settings), 0, MSM_CAMERA_I2C_WORD_DATA},
	{&dpc_settings[0], ARRAY_SIZE(dpc_settings), 0,
	 MSM_CAMERA_I2C_WORD_DATA},
};

static struct msm_camera_i2c_conf_array mt9p017_confs[] = {
	{&snap_settings[0], ARRAY_SIZE(snap_settings), 0,
	 MSM_CAMERA_I2C_WORD_DATA},
	{&prev_settings[0], ARRAY_SIZE(prev_settings), 0,
	 MSM_CAMERA_I2C_WORD_DATA},
	{&FHD_settings[0], ARRAY_SIZE(FHD_settings), 0,
	 MSM_CAMERA_I2C_WORD_DATA},
};
...
//                                                 
static int32_t mt9p017_i2c_read_w_eeprom(struct msm_camera_i2c_client
					 ...
					 uint16_t * rdata)
{
	...
	rc = msm_camera_i2c_rxdata_manual(sensor_i2c_client->client->adapter,
					  MT9P017_EEPROM_SLAVE_ADDR, &buf, 1);
	if (rc < 0) {
		printk(KERN_EMERG
		       "[CAMERA]1 :mt9p017_i2c_read_eeprom 0x%x failed!\n",
		       buf);
		return rc;
	}
	*rdata = buf;
	...
	if (rc < 0) {
		printk(KERN_EMERG
		       "[CAMERA]2 :mt9p017_i2c_read_eeprom 0x%x failed!\n",
		       buf);
		return rc;
	}
	*rdata = (*rdata << 8) | buf;
...
static int32_t mt9p017_read_5100k_data(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	for (j = 0; j < LSC_FIRST_PHASE1_DATA_SIZE; j++) {
		for (i = 0; i < LSC_FIRST_PHASE2_DATA_SIZE; i++) {
			rc = mt9p017_i2c_read_w_eeprom(s_ctrl->
						       sensor_i2c_client,
						       reg_addr_read,
						       &eepromdata);
			if (rc < 0) {
	...
	for (i = 0; i < LSC_SECOND_PHASE_DATA_SIZE; i++) {
		rc = mt9p017_i2c_read_w_eeprom(s_ctrl->sensor_i2c_client,
...
static int32_t mt9p017_read_awb_data(struct msm_sensor_ctrl_t *s_ctrl,
				     ...
				     *sensor_cablib_data, bool bresult)
{
	...
	reg_addr = 0xD4D5;	// R/G
	rc = mt9p017_i2c_read_w_eeprom(s_ctrl->sensor_i2c_client, reg_addr,
				       &eepromdata);
	if (rc < 0) {
		printk(KERN_EMERG
...
static int32_t mt9p017_get_pict_fps(struct msm_sensor_ctrl_t *s_ctrl, uint16_t fps, uint16_t * pfps)	// mo2jonghoo.lee 2012.12.27 
{
	...
	preview_frame_length_lines =
	    MT9P017_QTR_SIZE_HEIGHT + MT9P017_VER_QTR_BLK_LINES;
	preview_line_length_pck =
	    MT9P017_QTR_SIZE_WIDTH + MT9P017_HRZ_QTR_BLK_PIXELS;
	/* Total frame_length_lines and line_length_pck for snapshot */
	snapshot_frame_length_lines =
	    MT9P017_FULL_SIZE_HEIGHT + MT9P017_VER_FULL_BLK_LINES;
	snapshot_line_length_pck =
	    MT9P017_FULL_SIZE_WIDTH + MT9P017_HRZ_FULL_BLK_PIXELS;
					 ...
					 uint16_t * p_prevl_pf)
{
	...
	if (s_ctrl->curr_res == MSM_SENSOR_RES_QTR /* QTR_SIZE */ )
		...
		*p_prevl_pf =
		    MT9P017_FHD_SIZE_HEIGHT + MT9P017_VER_FHD_BLK_LINES;
					  ...
					  uint16_t * p_prevp_pl)
{
	...
	if (s_ctrl->curr_res == MSM_SENSOR_RES_QTR /* QTR_SIZE */ )
		...
		*p_prevp_pl =
		    MT9P017_FHD_SIZE_WIDTH + MT9P017_HRZ_FHD_BLK_PIXELS;
	else			//MSM_SENSOR_RES_FULL /* FULL_SIZE */
		*p_prevp_pl =
		    MT9P017_FULL_SIZE_WIDTH + MT9P017_HRZ_FULL_BLK_PIXELS;
					   ...
					   uint32_t * p_pict_max_exp_lc)
{
	...
	if (s_ctrl->curr_res == MSM_SENSOR_RES_QTR /* QTR_SIZE */ )
		*p_pict_max_exp_lc =
		    (MT9P017_QTR_SIZE_HEIGHT + MT9P017_VER_QTR_BLK_LINES) * 24;
	else if (s_ctrl->curr_res == MSM_SENSOR_RES_2 /* FHD_SIZE */ )
		*p_pict_max_exp_lc =
		    (MT9P017_FHD_SIZE_HEIGHT + MT9P017_VER_FHD_BLK_LINES) * 24;
	else			//MSM_SENSOR_RES_FULL /* FULL_SIZE */
		*p_pict_max_exp_lc =
		    (MT9P017_FULL_SIZE_HEIGHT +
...
//No on JB      } else {
	total_lines_per_frame =
	    (uint16_t) (total_lines_per_frame * s_ctrl->fps_divider / 0x400);
//No on JB      }
...
//No on JB      }
	...
	CDBG("[CAMERA][1] mt9p017_write_exp_gain : frame_length_lines = NONE, mt9p017_ctrl->fps_divider = %d, mt9p017_ctrl->pict_fps_divider = %d\n",
	     ...
	     -1 /* mt9p017_ctrl->pict_fps_divider */ );
					       ...
					       uint16_t gain, uint32_t line)
{
	...
	msm_camera_i2c_write(s_ctrl->sensor_i2c_client, REG_RESET_REGISTER,
			     REG_FAST_TRANS_MODE_ON, MSM_CAMERA_I2C_WORD_DATA);
				  ...
				  int direction, int32_t num_steps)
{
	...
	dest_step_position =
	    mt9p017_ctrl->curr_step_pos + (step_direction * num_steps);
	...
	target_dist =
	    step_direction * (dest_lens_position - mt9p017_ctrl->curr_lens_pos);
	//printk("Target Dist: %hd\n", target_dist);

	if (step_direction < 0 && (target_dist >=
				   mt9p017_step_position_table
				   [mt9p017_damping_threshold])) {
	...
	for (next_lens_position =
	     mt9p017_ctrl->curr_lens_pos + (step_direction * small_step);
...
static int32_t mt9p017_config(struct msm_sensor_ctrl_t *s_ctrl,
			      void __user * argp)
{
	...
	case CFG_GET_CALIB_DATA:
		rc = mt9p017_read_eeprom_data(s_ctrl, &cdata.cfg.calib_info);
	...
	case CFG_SET_DEFAULT_FOCUS:
		rc = mt9p017_set_default_focus(s_ctrl, cdata.cfg.focus.steps);
		break;
	...
	case CFG_GET_PREV_L_PF:
		rc = mt9p017_get_prev_lines_pf(s_ctrl, &cdata.cfg.prevl_pf);
	...
	case CFG_GET_PREV_P_PL:
		rc = mt9p017_get_prev_pixels_pl(s_ctrl, &cdata.cfg.prevp_pl);
	...
	case CFG_GET_PICT_L_PF:
		rc = mt9p017_get_pict_lines_pf(s_ctrl, &cdata.cfg.pictl_pf);
	...
	case CFG_GET_PICT_P_PL:
		rc = mt9p017_get_pict_pixels_pl(s_ctrl, &cdata.cfg.pictp_pl);
	...
	case CFG_SET_LENS_SHADING:	//                                                                    
		rc = mt9p017_lens_shading_enable(s_ctrl,
						 cdata.cfg.lens_shading);
		break;

	case CFG_GET_AF_MAX_STEPS:
		rc = mt9p017_get_af_max_steps(s_ctrl, &cdata.max_steps);
...
static int32_t mt9p017_actuator_move_focus(struct msm_actuator_ctrl_t *a_ctrl,
...
static int32_t mt9p017_actuator_set_default_focus(struct msm_actuator_ctrl_t
						  *a_ctrl,
						  struct
						  ...
						  *move_params)
{
	...
	if (mt9p017_ctrl->curr_step_pos != 0) {
		...
		rc = mt9p017_actuator_move_focus(a_ctrl, move_params);
	} else {
...
struct msm_actuator msm_actuator_table_mt9p017 = {
	...
	.func_tbl = {
		     ...
		     mt9p017_actuator_set_default_focus,
...
static struct msm_sensor_reg_t mt9p017_regs = {
	...
	.group_hold_off_conf_size = ARRAY_SIZE(mt9p017_groupoff_settings),
