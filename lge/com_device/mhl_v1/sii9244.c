...
#ifdef CONFIG_LGE_PM
	wait_ret =
	    wait_for_completion_timeout(&conv_complete_evt,
					msecs_to_jiffies
					(MSM_PMIC_ADC_READ_TIMEOUT));
	if (wait_ret <= 0) {
		printk(KERN_ERR
		       "===%s: failed to adc wait for completion!===\n",
		       __func__);
		goto sanity_out;
	}
#else
...
void sii9244_cfg_power(bool on)
{
	if (mhl_data.initialized == 0) {
		pr_err("%s: mhl_data.initialized = %d\n", __func__,
		       mhl_data.initialized);
		return;
	}

	if (mhl_data.power == on) {
		pr_info("%s : alread %s\n", __func__,
			(mhl_data.power ? "on" : "off"));
		return;
	}
...
void mhl_delayed_pwron_request(unsigned int delay_ms)
{
	...
	mutex_unlock(&mhl_data.lock);
	queue_delayed_work(mhl_data.wq, &mhl_data.dwork,
...
void sii9244_workfunc_pwron(struct work_struct *p)
{
	...
	for (retry = 0; retry < 1; retry++) {
		...
		if ((acc_adc >= -100 && acc_adc <= 100) && (acc_adc != -EINVAL)) {
			pr_info
			    ("%s: MHL Cable Detected : MHL_PWRON_DELAY_MS[%d]... retry[%d]\n",
		...
		{
			if (check_usb_online()
			    && sii9244_get_last_rsen_incorrect()) {
				pr_info
				    ("%s: in rsen incorrect state ###########\n",
				     __func__);
				mhl_delayed_pwron_request(100);
			} else
				sii9244_clear_rsen_incorrect();
...
static int mhl_virtual_kbmouse_register(void)	// mouse & keyboard emulator...
{
	...
	mhl_virtual_kbmouse->keybit[BIT_WORD(BTN_MOUSE)] =
...
static int __devinit sii9244_p0_i2c_probe(struct i2c_client *client,
...
static int __devinit sii9244_p1_i2c_probe(struct i2c_client *client,
					  const struct i2c_device_id *id)
{
...
static int __devinit sii9244_p2_i2c_probe(struct i2c_client *client,
					  const struct i2c_device_id *id)
{
...
static int __devinit sii9244_p3_i2c_probe(struct i2c_client *client,
					  const struct i2c_device_id *id)
{
...
#ifdef CONFIG_PM
static int sii9244_suspend(struct platform_device *pdev, pm_message_t state)
{
	if (mhl_data.pdata->is_support == 0)
		return 0;
