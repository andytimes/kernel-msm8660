...
static int lm3533_write_reg(struct i2c_client *client, unsigned char reg,
			    unsigned char val);
...
//                                               
			...
			lm3533_write_reg(main_lm3533_dev->client, 0x40,
					 cal_value);
		} else {
			cal_value = level;
...
void lm3533_backlight_off(void)
{
	...
	gpio_tlmm_config(GPIO_CFG
			 (gpio, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP,
			  GPIO_CFG_2MA), GPIO_CFG_ENABLE);
...
static ssize_t touchkey_backlight_show_level(struct device *dev,
					     struct device_attribute *attr,
					     char *buf)
{
	int r;
...
static ssize_t touchkey_backlight_store_level(struct device *dev,
					      struct device_attribute *attr,
					      const char *buf, size_t count)
{
	if (!count)
...
DEVICE_ATTR(lm3533_touchkeyled_control, 0664, touchkey_backlight_show_level,
	    touchkey_backlight_store_level);
			...
			const struct i2c_device_id *id)
{
	...
	if (lge_bd_rev >= LGE_REV_A) {
		...
		bl_dev =
		    backlight_device_register(I2C_BL_NAME, &i2c_dev->dev, NULL,
					      &lm3533_bl_ops, &props);
	...
	err =
	    device_create_file(&i2c_dev->dev,
			       &dev_attr_lm3533_backlight_on_off);
	err =
	    device_create_file(&i2c_dev->dev,
			       &dev_attr_lm3533_touchkeyled_control);
