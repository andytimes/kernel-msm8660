...
static int lm3537_write_reg(struct i2c_client *client, unsigned char reg,
			    unsigned char val);
...
static void lm3537_set_main_current_level(struct i2c_client *client, int level)
{
	...
	if (level < 22)
		cal_value = 55;
	else if ((level >= 22) & (level < 84))
		cal_value = ((level * 3) / 5) + 63;
	else
		cal_value = (level - 51) * (max_brightness - 107) / 105 + 107;

#ifdef CONFIG_LGE_PM_FACTORY_CURRENT_DOWN
	if ((usb_cable_info == 6) || (usb_cable_info == 7)
	    || (usb_cable_info == 11)) {
...
DEVICE_ATTR(lm3537_level, 0644, lcd_backlight_show_level,
	    lcd_backlight_store_level);
DEVICE_ATTR(lm3537_backlight_on_off, 0644, lcd_backlight_show_on_off,
...
static int lm3537_probe(struct i2c_client *i2c_dev,
			const struct i2c_device_id *id)
{
	...
	err =
	    device_create_file(&i2c_dev->dev,
			       &dev_attr_lm3537_backlight_on_off);
