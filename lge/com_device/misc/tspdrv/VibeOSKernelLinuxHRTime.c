...
 static int VibeOSKernelProcessData(void *data) 
{
		    ...
		    &(g_SamplesBuffer[i]);
				...
				0};
				    ...
				    ImmVibeSPI_ForceOut_SetSamples(i, 8, 1,
								   cZero);
				...
				(pCurrentActuatorSample->
				 actuatorSamples[(int)pCurrentActuatorSample->
						 nIndexPlayingBuffer].
				 nActuatorIndex,
				 pCurrentActuatorSample->
				 actuatorSamples[(int)pCurrentActuatorSample->
						 nIndexPlayingBuffer].nBitDepth,
				 ...
				 dataBuffer))
				    ...
				    hrtimer_forward_now(&g_tspTimer,
							g_ktFiveMs);
				}
			 pCurrentActuatorSample->nIndexOutputValue +=
			    pCurrentActuatorSample->
			    actuatorSamples[(int)pCurrentActuatorSample->
					    nIndexPlayingBuffer].nBufferSize;
...
}  static void VibeOSKernelLinuxStartTimer(void) 
{
	    ...
	    g_nWatchdogCounter = 0;
		    ...
		    for (i = 0; i < NUM_ACTUATORS; i++)
			     ...
			     || (g_SamplesBuffer[i].actuatorSamples[1].
				 nBufferSize))
