...
static struct file_operations fops = 
    { .owner = THIS_MODULE, .read = read, .write = write, 
	    ...
	    .open = open, .release = release, .llseek =
	    default_llseek
	    /* using default implementation as declared in linux/fs.h */  
};
...
int __init tspdrv_init(void) 
{
	    ...
	    nRet = misc_register(&miscdev);
			 ...
			 "tspdrv: platform_device_register failed.\n"));
		}
	 nRet = platform_driver_register(&platdrv);
	if (nRet)
		 {
		DbgOut((KERN_ERR
			 "tspdrv: platform_driver_register failed.\n"));
...
 static ssize_t read(struct file *file, char *buf, size_t count,
		       loff_t * ppos) 
{
	const size_t nBufSize =
	    (g_cchDeviceName > (size_t) (*ppos)) ? min(count,
						       g_cchDeviceName -
						       (size_t) (*ppos)) : 0;
			...
			loff_t * ppos) 
{
		...
		 samples_buffer * pInputBuffer =
		    (samples_buffer *) (&g_cWriteBuffer[i]);
...
#error "SPI_HEADER_SIZE expected to be 3"
		    ...
		    /* Check buffer size */ 
		    if ((i + SPI_HEADER_SIZE + pInputBuffer->nBufferSize) >
			count)
