	...
	if (NULL == snddev_reg_l1) {
		...
		if (IS_ERR(snddev_reg_l1)) {
			pr_err("LGE: VIB %s: regulator_get(%s) failed (%ld)\n",
			       __func__, "l1", PTR_ERR(snddev_reg_l1));
			return -EBUSY;
		}
	}
	if (enable == snddev_reg_l1_status)
		return 0;
	if (enable) {
		rc = regulator_set_voltage(snddev_reg_l1, VIBE_IC_VOLTAGE,
					   VIBE_IC_VOLTAGE);
		if (rc < 0)
			pr_err
			    ("LGE:  VIB %s: regulator_set_voltage(l1) failed (%d)\n",
			     __func__, rc);
		...
		if (rc < 0)
			pr_err
			    ("LGE: VIB %s: regulator_enable(l1) failed (%d)\n",
			     __func__, rc);
...
//      printk("[vibrator] %s is called \n", __func__);
	...
	writel(0x0002, vib_base_ptr + GP_MN_CLK_MDIV_REG);
	writel(0x1AE9, vib_base_ptr + GP_MN_CLK_NDIV_REG);
...
//                                                      
	...
	if (enable) {
		...
		REG_WRITEL((((M_VAL & 0xffU) << 16U) +	/* M_VAL[23:16] */
			    ((~(D_VAL << 1)) & 0xffU)),	/* D_VAL[7:0] */
			   GP1_MD_REG);
		REG_WRITEL(((((~(N_VAL - M_VAL)) & 0xffU) << 16U) +	/* N_VAL[23:16] */
		...
		gpio_tlmm_config(GPIO_CFG
				 (GPIO_LIN_MOTOR_PWM, 2, GPIO_CFG_OUTPUT,
				  GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
				 GPIO_CFG_ENABLE);
		if (D_VAL > PWM_MAX_DUTY)
			D_VAL = PWM_MAX_DUTY;
		if (D_VAL < PWM_MIN_DUTY)
			D_VAL = PWM_MIN_DUTY;
...
//                                                                       
		...
		for (val_cnt = 0; val_cnt < 3; val_cnt++) {
			...
			if (reg_val == 0x0002) {
				...
				printk(KERN_INFO
				       "LGE_ON: %s[val_cnt = %d]  Read GP Mn MDIV reg val 0x%X\n",
				       __func__, val_cnt, reg_val);
				writel(0x0002,
				       vib_base_ptr + GP_MN_CLK_MDIV_REG);
			}
		}
		...
		for (val_cnt = 0; val_cnt < 3; val_cnt++) {
			...
			if (reg_val == 0x1AE9) {
				...
				printk(KERN_INFO
				       "LGE_ON: %s[val_cnt = %d]  Read GP Mn NDIV reg val 0x%X\n",
				       __func__, val_cnt, reg_val);
				writel(0x1AE9,
				       vib_base_ptr + GP_MN_CLK_NDIV_REG);
			}
		}
...
//            printk("[Vibrator] GPIO_LIN_MOTOR_PWM is enable with M=%d N=%d D=%d\n", M_VAL, N_VAL, D_VAL);
		...
		REG_WRITEL(((((~(N_VAL - M_VAL)) & 0xffU) << 16U) +	/* N_VAL[23:16] */
			   ...
			   GP1_NS_REG);
#else
		writel(GP_MN_D_DEFAULT & GPMN_D_MASK,
		       vib_base_ptr + GP_MN_CLK_DUTY_REG);

		gpio_tlmm_config(GPIO_CFG
				 (GPIO_LIN_MOTOR_PWM, 0, GPIO_CFG_INPUT,
