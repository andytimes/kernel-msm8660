...
static void melfas_ts_work_func(struct work_struct *work)
{
	...
	int touchType = 0, touchState = 0, touchID = 0, posX = 0, posY =
	    0, width = 0, strength = 10, keyID = 0, reportID = 0;
	uint8_t read_num;
...
#if DEBUG_PRINT
			printk(KERN_ERR
			       "melfas_ts_work_func : i2c_master_recv [%d]\n",
			       ret);
...
#if 1				//DEBUG_PRINT
				if ((touchState == 1 && tmp_flag[touchID] == 1)
				    || (touchState == 0
					&& tmp_flag[touchID] == 1)) {
					printk(KERN_ERR
...
#if 1				//DEBUG_PRINT
			printk(KERN_ERR
			       "melfas_ts_work_func: keyID : %d, touchState: %d\n",
			       keyID, touchState);
#endif
		}
	...
	for (i = 0; i < read_num; i = i + 6)	// extract touch information
	{
		if (ret < 0) {
			...
			if (touchType == TOUCH_SCREEN) {
				...
				if (touchState)
					g_Mtouch_info[touchID].strength =
					    strength;
				else {
					g_Mtouch_info[touchID].strength = 0;
					tmp_flag[touchID] = 1;
...
#if DEBUG_PRINT
			if ((touchState == 1 && tmp_flag[touchID] == 1)
			    || (touchState == 0 && tmp_flag[touchID] == 1)) {
				printk(KERN_ERR
				       "Touch ID: %d, State : %d, x: %d, y: %d, z: %d w: %d\n",
			   ...
			   const struct i2c_device_id *id)
{
	...
	if (!melfas_wq) {
		printk(KERN_ERR
		       "[TOUCH]failed to create singlethread workqueue\n");
		return -ENOMEM;
	}
#if 1				//DEBUG_PRINT
	...
	if (ts == NULL) {
		printk(KERN_ERR
		       "melfas_ts_probe: failed to create a state of melfas-ts\n");
		ret = -ENOMEM;
	...
	if (ts->pdata->power_enable)
		...
		printk(KERN_ERR
		       "melfas_ts_probe: ts->pdata->power_enable is NULL \n");
		ret = -ENOMEM;
		goto err_alloc_data_failed;
	}
	...
	for (i = 0; i < I2C_RETRY_CNT; i++) {
		...
		if (ret >= 0) {
			printk(KERN_ERR
			       "melfas_ts_probe: i2c_master_send() ok [%d]\n",
			       ret);
			break;
		} else
			printk(KERN_ERR
			       "melfas_ts_probe: i2c_master_send() failed[%d]\n",
			       ret);
	}
...
//      ts->input_dev->evbit[0] =  BIT_MASK(EV_SYN) | BIT_MASK(EV_ABS) | BIT_MASK(EV_KEY);
	...
	input_set_abs_params(ts->input_dev, ABS_MT_POSITION_Y, 0,
			     TS_MAX_Y_COORD, 0, 0);
	input_set_abs_params(ts->input_dev, ABS_MT_PRESSURE, 0, TS_MAX_Z_TOUCH,
			     0, 0);
...
//      __set_bit(EV_KEY, ts->input_dev->evbit);
	...
	if (ts->client->irq) {
#if DEBUG_PRINT
		printk(KERN_ERR
		       "melfas_ts_probe: trying to request irq: %s-%d\n",
		...
		if (ret > 0) {
			printk(KERN_ERR
			       "melfas_ts_probe: Can't allocate irq %d, ret %d\n",
			       ts->client->irq, ret);
			ret = -EBUSY;
...
#if DEBUG_PRINT
	printk(KERN_INFO
	       "melfas_ts_probe: Start touchscreen. name: %s, irq: %d\n",
	       ts->client->name, ts->client->irq);
#endif
	return 0;
...
static void melfas_ts_suspend_func(struct melfas_ts_data *ts)
{
	...
	if (ret < 0)
		printk(KERN_ERR
		       "melfas_ts_suspend: i2c_smbus_write_byte_data failed\n");
