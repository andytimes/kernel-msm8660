...
static unsigned int synaptics_rmi4_i2c_debug_mask =
    SYNAPTICS_RMI4_I2C_DEBUG_BUTTON_STATUS | SYNAPTICS_RMI4_I2C_DEBUG_NONE;
...
typedef struct {
	 ...
	 */
	unsigned char
	    finger_data[MAX_NUM_OF_FINGER][NUM_OF_EACH_FINGER_DATA_REG];
} ts_sensor_data;
...
static void synaptics_ts_soft_reset(struct synaptics_ts_data *ts)
{
	...
	queue_delayed_work(synaptics_wq,
			   &ts->work,
			   msecs_to_jiffies(ts->pdata->ic_booting_delay));
}
...
static void synaptics_ts_hard_reset(struct synaptics_ts_data *ts)
{
	 ...
	 */
	if (ts->pdata->power) {
		if (synaptics_rmi4_i2c_debug_mask &
...
static void synaptics_ts_fw_upgrade(struct synaptics_ts_data *ts)
{
	...
	if (likely(!ts->is_downloading)) {
		...
		if (synaptics_rmi4_i2c_debug_mask &
			...
			memset(&time_debug, 0x0,
			       sizeof(struct synaptics_ts_timestamp));
		...
		if (synaptics_rmi4_i2c_debug_mask &
			...
			if (atomic_read(&time_debug.ready) == 1) {
				...
				time_debug.result_t =
				    time_debug.end - time_debug.start;
				time_debug.rem =
				    do_div(time_debug.result_t, 1000000000);
				SYNAPTICS_DEBUG_MSG
				    ("FW upgrade time < %2lu.%06lu\n",
				     (unsigned long)time_debug.result_t,
				     time_debug.rem / 1000);
		...
		SYNAPTICS_ERR_MSG
		    ("Firmware Upgrade process is aready working on\n");
	}
}
#endif
				...
				struct device_attribute *attr, char *buf)
{
	...
	ret =
	    sprintf(buf, "Touch IC Chip Info\n - Product ID: %s\n",
		    ts->product_id);
	ret +=
	    sprintf(buf + ret, " - Manufacturer ID: %d, FW revision: %d\n",
		    ts->manufcturer_id, ts->fw_rev);
#if defined(CONFIG_LGE_TOUCH_SYNAPTICS_FW_UPGRADE)
	ret +=
	    sprintf(buf + ret,
		    " - Kernel wished FW revision: %d, FW binary revision: %d\n",
			     ...
			     size_t count)
{
	...
	if (!strncmp(string, "interrupt", 10)) {
		...
		SYNAPTICS_INFO_MSG
		    ("WARNING: You can use this mode change only interrupt mode booting\n");
		SYNAPTICS_INFO_MSG
		    ("Usage: echo [interrupt | polling] > ts_mode\n");
		SYNAPTICS_INFO_MSG
		    (" - interrupt : Change to interrupt mode start\n");
			    ...
			    struct device_attribute *attr, char *buf)
{
	...
	ret =
	    sprintf(buf, "Palm Detect threshold (palm_threshold)\t\t\t: 0x%x\n",
		    ts->pdata->palm_threshold);
	ret +=
	    sprintf(buf + ret,
		    "Delta Position threshold (delta_pos_threshold)\t\t: 0x%x\n",
			     ...
			     size_t count)
{
	...
	if (!strncmp(string, "palm", 4)) {
		ret =
		    i2c_smbus_write_byte_data(ts->client, PALM_DETECT_REG,
					      (u8) value);
		if (ret < 0) {
		...
		ret =
		    i2c_smbus_write_byte_data(ts->client, DELTA_X_THRESH_REG,
					      (u8) value);
		if (ret < 0) {
			...
			SYNAPTICS_INFO_MSG
			    ("Delta X threshold is setted to 0x%x\n",
			     (u8) value);
			ts->pdata->delta_pos_threshold = value;
		}

		ret =
		    i2c_smbus_write_byte_data(ts->client, DELTA_Y_THRESH_REG,
					      (u8) value);
		if (ret < 0) {
			...
			SYNAPTICS_INFO_MSG
			    ("Delta Y threshold is setted to 0x%x\n",
			     (u8) value);
			ts->pdata->delta_pos_threshold = value;
		}
	} else {
		SYNAPTICS_INFO_MSG
		    ("Usage: echo [palm | delta] value(16)> ts_sens\n");
		SYNAPTICS_INFO_MSG
		    (" - palm : palm threshold register setting\n");
		SYNAPTICS_INFO_MSG
		    (" - delta : X,Y delta threshold register setting\n");
	}
			    ...
			    struct device_attribute *attr, char *buf)
{
	...
	ret += sprintf(buf + ret, "Operation mode (use_irq)\t\t\t\t: %s (%d)\n",
		       ts->pdata->use_irq ? "Interrupt" : "Polling",
		       ts->pdata->use_irq);
	ret +=
	    sprintf(buf + ret, "IRQ flag (irqflags)\t\t\t\t\t: %ld\n",
		    ts->pdata->irqflags);
	ret +=
	    sprintf(buf + ret, "I2C SDA (i2c_sda_gpio)\t\t\t\t\t: %d\n",
		    ts->pdata->i2c_sda_gpio);
	ret +=
	    sprintf(buf + ret, "I2C SCL (ni2c_scl_gpio)\t\t\t\t\t: %d\n",
		    ts->pdata->i2c_scl_gpio);
	ret +=
	    sprintf(buf + ret, "I2C Interrupt (i2c_int_gpio)\t\t\t\t: %d\n",
		    ts->pdata->i2c_int_gpio);
	ret +=
	    sprintf(buf + ret, "IC booting delay (booting_delay)\t\t\t: %dms\n",
		    ts->pdata->ic_booting_delay);
	ret +=
	    sprintf(buf + ret,
		    "IC interrupt interval (report_period)\t\t\t: %ldns\n",
		    ts->pdata->report_period);
	ret +=
	    sprintf(buf + ret, "Number of Finger (num_of_finger)\t\t\t: %d\n",
		    ts->pdata->num_of_finger);
	ret +=
	    sprintf(buf + ret, "Number of Button (num_of_button)\t\t\t: %d\n",
		    ts->pdata->num_of_button);
	ret += sprintf(buf + ret, "Button key code\t\t\t\t\t\t: ");
	if (ts->pdata->num_of_button != 0) {
		for (i = 0; i < ts->pdata->num_of_button; i++) {
			ret +=
			    sprintf(buf + ret, "button[%d] - %d", i,
				    ts->pdata->button[i]);
	...
	ret +=
	    sprintf(buf + ret,
		    "Touch 2D area\t\t\t\t\t\t: X - 0 ~ %d (x_max)\n",
		    ts->pdata->x_max);
	ret +=
	    sprintf(buf + ret, "\t\t\t\t\t\t\t  Y - 0 ~ %d (y_max)\n",
		    ts->pdata->y_max);
	ret +=
	    sprintf(buf + ret, "Needed FW revision (fw_ver)\t\t\t\t: %d\n",
		    ts->pdata->fw_ver);
	ret +=
	    sprintf(buf + ret,
		    "Palm Detect threshold (palm_threshold)\t\t\t: 0x%x\n",
		    ts->pdata->palm_threshold);
	ret +=
	    sprintf(buf + ret,
		    "Delta Position threshold (delta_pos_threshold)\t\t: 0x%x\n",
	...
	ret +=
	    sprintf(buf + ret,
		    "Download process (is_downloading)\t\t\t: %s (%d)\n",
		    ts->is_downloading ? "Working" : "Idle",
		    ts->is_downloading);
	ret +=
	    sprintf(buf + ret,
		    "Suspend status (is_suspended)\t\t\t\t: %s (%d)\n",
		    ts->is_suspended ? "Suspend" : "Idle", ts->is_suspended);
	ret +=
	    sprintf(buf + ret,
		    "Button previous status (button_prestate)\t\t: ");
	for (i = 0; i < ts->pdata->num_of_button; i++) {
	...
	ret +=
	    sprintf(buf + ret,
		    "Finger previous status (finger_prestate)\t\t: ");
	...
	ret +=
	    sprintf(buf + ret,
		    "Driver Probe status (is_probed)\t\t\t\t: %s (%d)\n",
		    ts->is_probed ? "OK" : "Fail", ts->is_probed);
	ret +=
	    sprintf(buf + ret,
		    "No Melt mode setting (melt_mode)\t\t\t: %s (%d)\n",
		    ts->melt_mode ? "No Melt" : "Melt", ts->melt_mode);
	ret +=
	    sprintf(buf + ret,
		    "No melt setting drag distance (idle_lock_distance)\t: %d\n",
		    ts->idle_lock_distance);
	i = atomic_read(&ts->interrupt_handled);
	ret +=
	    sprintf(buf + ret,
		    "Interrupt status (interrupt_handled)\t\t\t: %s (%d)\n",
		    i ? "Not Handled" : "Handled", i);
	ret +=
	    sprintf(buf + ret,
		    "IC supported Function\t\t\t\t\t: $%02x, $%02x, $%02x\n",
		    ts->common_dsc.id, ts->finger_dsc.id, ts->button_dsc.id);
	ret +=
	    sprintf(buf + ret,
		    "Finger Interrupt bit mask (int_status_reg_asb0_bit)\t: 0x%02x\n",
		    ts->int_status_reg_asb0_bit);
	ret +=
	    sprintf(buf + ret,
		    "Button Interrupt bit mask (int_status_reg_button_bit)\t: 0x%02x\n",
		    ts->int_status_reg_button_bit);
	ret +=
	    sprintf(buf + ret, "Current Finger position (pre_ts_data)\t\t\t: ");
	for (i = 0; i < ts->pdata->num_of_finger; i++) {
		ret +=
		    sprintf(buf + ret, "(%d, %d, %d)", ts->pre_ts_data.pos_x[i],
			    ts->pre_ts_data.pos_y[i],
			    ts->pre_ts_data.pressure[i]);
		if (i != 0 && (i + 1) % 2 == 0
		    && i != ts->pdata->num_of_finger - 1)
			ret += sprintf(buf + ret, "\n\t\t\t\t\t\t\t  ");
	...
	ret += sprintf(buf + ret, "\n");
	ret +=
	    sprintf(buf + ret, "IC FW Revision (fw_rev)\t\t\t\t\t: %d\n",
		    ts->fw_rev);
	ret +=
	    sprintf(buf + ret, "IC Manufacture ID (manufcturer_id)\t\t\t: %d\n",
		    ts->manufcturer_id);
	ret +=
	    sprintf(buf + ret, "IC Product ID (product_id)\t\t\t\t: %s\n",
		    ts->product_id);
...
static void read_page_description_table(struct synaptics_ts_data *ts)
{
	...
	for (u_address = DESCRIPTION_TABLE_START; u_address > 10;
	     u_address -= sizeof(ts_function_descriptor)) {
		ret =
		    synaptics_ts_read(ts->client, u_address, sizeof(buffer),
...
static void synaptics_ts_button_lock_work_func(struct work_struct
					       *button_lock_work)
{
	struct synaptics_ts_data *ts =
	    container_of(to_delayed_work(button_lock_work),
			 struct synaptics_ts_data, button_lock_work);
	...
	ret =
	    i2c_smbus_write_byte_data(ts->client, INTERRUPT_ENABLE_REG,
				      ts->curr_int_mask);
}
...
static void synaptics_ts_work_func(struct work_struct *work)
{
	...
	if (ts->ic_init) {
		...
		ret = synaptics_ts_read(ts->client, DEVICE_STATUS_REG,
					...
					(u8 *) & ts_reg_ctrl.device_status_reg);
		if (ret < 0) {
...
&& (atomic_read(&ts->int_delay.ready) == 1)) {
				...
				ts->int_delay.result_t =
				    ts->int_delay.end - ts->int_delay.start;
				ts->int_delay.rem =
				    do_div(ts->int_delay.result_t, 1000000);
				SYNAPTICS_DEBUG_MSG
				    ("Touch IC interrupt line clear time < %3lu.%03lu\n",
				     (unsigned long)ts->int_delay.result_t,
				     ts->int_delay.rem / 1000);
			}
		}
		...
		/* ESD damage check */
		if ((ts_reg_ctrl.device_status_reg & DEVICE_FAILURE_MASK) ==
		    DEVICE_FAILURE_MASK) {
			SYNAPTICS_ERR_MSG
			    ("ESD damage occured. Reset Touch IC\n");
			ts->ic_init = 0;
			synaptics_ts_hard_reset(ts);
			return;
		}
		...
		if (((ts_reg_ctrl.
		      device_status_reg & DEVICE_STATUS_REG_UNCONFIGURED) >> 7)
		    == 1) {
			SYNAPTICS_ERR_MSG
			    ("Touch IC resetted internally. Reconfigure register setting\n");
			ts->ic_init = 0;
			queue_delayed_work(synaptics_wq, &ts->work, 0);
			return;
		}
		...
		ret = synaptics_ts_read(ts->client, INTERRUPT_ENABLE_REG,
					sizeof(ts->curr_int_mask),
					(u8 *) & ts->curr_int_mask);

		if (ts_reg_ctrl.interrupt_status_reg & ts->int_status_reg_asb0_bit && ts->curr_int_mask & ts->int_status_reg_asb0_bit) {	/* Finger */
...
/* FIXME: it will move to Framework and be removed */
			if (ts_reg_data.finger_state_reg[0] == 1
				...
				if (ret < 0)
					SYNAPTICS_ERR_MSG
					    ("DELTA_Y_THRESH_REG write fail\n");
			}

			for (f_counter = 0;
			     f_counter < ts->pdata->num_of_finger;
			     f_counter++) {
				reg_num = f_counter / 4;
				...
				if (synaptics_rmi4_i2c_debug_mask &
					...
					if (finger_order == 0)
						SYNAPTICS_DEBUG_MSG
						    ("Finger status register%d: 0x%x\n",
						     reg_num,
						     ts_reg_data.
						     finger_state_reg[reg_num]);
				}

				if (((ts_reg_data.
				      finger_state_reg[reg_num] >> (finger_order
								    * 2))
				     & FINGER_STATE_MASK) == 1) {
					curr_ts_data.pos_x[f_counter] =
					    (int)GET_X_POSITION(ts_reg_data.
								finger_data
								[f_counter]
								[REG_X_POSITION],
								ts_reg_data.
					...
					curr_ts_data.pos_y[f_counter] =
					    (int)GET_Y_POSITION(ts_reg_data.
								finger_data
								[f_counter]
								[REG_Y_POSITION],
								ts_reg_data.
					...
					if (((ts_reg_data.
					      finger_data[f_counter][REG_WY_WX]
					      & 0xF0) >> 4)
					    > (ts_reg_data.
					       finger_data[f_counter][REG_WY_WX]
					       & 0x0F)) {
					...
					if (ts->finger_prestate[f_counter] ==
					    TOUCH_RELEASED) {
						ts->finger_prestate[f_counter] =
						    TOUCH_PRESSED;
					 ...
					 */
					if (f_counter == 0 && ts->melt_mode == 0 && ts_reg_data.finger_state_reg[0] == 1	/* only 1 finger is pressed */
					    && ts_reg_data.
					    finger_state_reg[1] == 0
					    && ts_reg_data.
					    finger_state_reg[2] == 0) {

						if (ts->pre_ts_data.pos_x[0] !=
						    0
						    || ts->pre_ts_data.
						    pos_y[0] != 0) {
							ts->idle_lock_distance
					...
					ts->pre_ts_data.pos_x[f_counter] =
					    curr_ts_data.pos_x[f_counter];
					ts->pre_ts_data.pos_y[f_counter] =
					    curr_ts_data.pos_y[f_counter];
					ts->pre_ts_data.pressure[f_counter] =
					    curr_ts_data.pressure[f_counter];
				} else if (ts->finger_prestate[f_counter] ==
					   TOUCH_PRESSED) {
					if (synaptics_rmi4_i2c_debug_mask &
						...
						if (f_counter == 0 && ts->melt_mode == 0) {	/* only 1 finger is pressed */
							if (ts->
								...
								ret =
								    ...
								    (ts->client,
								     MELT_CONTROL_REG,
								     MELT_CONTROL_REG_NO_MELT);
								if (ret < 0) {
									SYNAPTICS_ERR_MSG
									    ("Melt mode setting fail\n");
								} else {
									ts->melt_mode = 1;
									if (synaptics_rmi4_i2c_debug_mask & SYNAPTICS_RMI4_I2C_DEBUG_FUNC_TRACE)
										SYNAPTICS_ERR_MSG
										    ("Melt mode setting success\n");
								}
							} else
								ts->idle_lock_distance = 0;
						}
     ...
     SYNAPTICS_RMI4_I2C_DEBUG_BUTTON_HANDLE_TIME)) {
				if (atomic_read(&ts->int_delay.ready) == 1) {
					...
					ts->int_delay.result_t =
					    ...
					    ts->int_delay.start;
					ts->int_delay.rem =
					    do_div(ts->int_delay.result_t,
						   1000000);
					SYNAPTICS_DEBUG_MSG
					    ("Touch Finger data report done time < %3lu.%03lu\n",
					     (unsigned long)ts->int_delay.
					     result_t,
					     ts->int_delay.rem / 1000);
				}
			}
		...
		if (likely(ts->button_dsc.id != 0)) {
			if (ts_reg_ctrl.interrupt_status_reg & ts->int_status_reg_button_bit && ts->curr_int_mask & ts->int_status_reg_button_bit) {	/* Button */

				if (synaptics_rmi4_i2c_debug_mask &
				    SYNAPTICS_RMI4_I2C_DEBUG_BUTTON_REG)
					SYNAPTICS_DEBUG_MSG
					    ("Button register: 0x%x\n",
				 ...
				 */
				for (b_counter = 0;
				     b_counter < ts->pdata->num_of_button;
				     b_counter++) {
...
&& (synaptics_rmi4_i2c_debug_mask &
    SYNAPTICS_RMI4_I2C_DEBUG_BUTTON_HANDLE_TIME)) {
					if (atomic_read(&ts->int_delay.ready) ==
					    1) {
						ts->int_delay.end =
						    cpu_clock(smp_processor_id
							      ());
						...
						ts->int_delay.rem =
						    do_div(ts->int_delay.
							   result_t, 1000000);
						SYNAPTICS_DEBUG_MSG
						    ("Touch Button data report done time < %3lu.%03lu\n",
						     (unsigned long)ts->
						     int_delay.result_t,
						     ts->int_delay.rem / 1000);
					}
...
exit_work:
		...
		if (ts->pdata->use_irq != 0) {
			if (gpio_get_value(ts->pdata->i2c_int_gpio) != 1
				...
				ret =
				    synaptics_ts_read(ts->client,
						      INTERRUPT_STATUS_REG, 1,
						      &temp);
				if (ret < 0)
					SYNAPTICS_ERR_MSG
					    ("INTERRUPT_STATUS_REG read fail\n");
				 ...
				 */
				SYNAPTICS_ERR_MSG
				    ("WARNING - Safety: Interrupt line isn't set high on time cause unexpected incident\n");
			}
		}
		...
		if (!ts->is_probed) {
			...
			if (likely(ts->button_dsc.id != 0)) {
				if (ts->finger_dsc.data_base >
				    ts->button_dsc.data_base) {
			...
			ret =
			    synaptics_ts_read(ts->client, MANUFACTURER_ID_REG,
					      1, &ts->manufcturer_id);
			if (ret < 0)
				SYNAPTICS_ERR_MSG("Manufcturer ID read fail\n");

			ret =
			    synaptics_ts_read(ts->client, FW_REVISION_REG, 1,
					      &ts->fw_rev);
			if (ret < 0)
				SYNAPTICS_ERR_MSG("FW revision read fail\n");

			ret =
			    synaptics_ts_read(ts->client, PRODUCT_ID_REG, 10,
					      &ts->product_id[0]);
			if (ret < 0)
...
#if defined(TEST_BOOTING_TIME_FW_FORCE_UPGRADE)
			{
				...
				SYNAPTICS_INFO_MSG
				    ("Synaptics %s(%s): Manufacturer ID=%d, FW revision=%d\n",
				     ts->product_id,
				     ts->pdata->
				     use_irq ? "Interrupt mode" :
				     "Polling mode", ts->manufcturer_id,
				     ts->fw_rev);
			}
			...
			if (((ts->pdata->fw_ver != ts->fw_rev)	/* Touch IC FW revision check */
				...
				if (likely
				    ...
				    if (unlikely
					(ts->fw_rev != get_fw_image_rev())) {
					SYNAPTICS_INFO_MSG
					    ("There is no suitable Touch F/W image file\n");
				}
			}
			...
			SYNAPTICS_INFO_MSG
			    ("Synaptics %s(%s): Manufacturer ID=%d, FW revision=%d\n",
			     ts->product_id,
			     ts->pdata->
			     use_irq ? "Interrupt mode" : "Polling mode",
			     ts->manufcturer_id, ts->fw_rev);
		...
		ret =
		    i2c_smbus_write_byte_data(ts->client, GESTURE_ENABLE_1_REG,
					      0x00);
		if (ret < 0)
			SYNAPTICS_ERR_MSG("GESTURE_ENABLE_1_REG write fail\n");
		ret =
		    i2c_smbus_write_byte_data(ts->client, GESTURE_ENABLE_2_REG,
					      0x00);
		if (ret < 0)
			SYNAPTICS_ERR_MSG("GESTURE_ENABLE_2_REG write fail\n");

		ret =
		    i2c_smbus_write_byte_data(ts->client, TWO_D_REPORTING_MODE,
		...
		/* interrupt line to high by Touch IC */
		ret =
		    synaptics_ts_read(ts->client, INTERRUPT_STATUS_REG, 1,
				      &temp);
		if (ret < 0)
			SYNAPTICS_ERR_MSG("INTERRUPT_STATUS_REG read fail\n");
	...
	if (likely(ts->ic_init)) {
		...
		if ((synaptics_rmi4_i2c_debug_mask &
			...
			if ((synaptics_rmi4_i2c_debug_mask &
				...
				if (atomic_read(&ts->int_delay.ready) == 1
				    && atomic_read(&ts->interrupt_handled) ==
				    1) {
					ts->int_delay.end =
					    cpu_clock(smp_processor_id());
					ts->int_delay.start = ts->int_delay.end;
			      ...
			      const struct i2c_device_id *id)
{
	...
	ts->pdata = pdata;
	ts->button_width =
	    (ts->pdata->x_max - (ts->pdata->num_of_button - 1) * BUTTON_MARGIN)
	    / ts->pdata->num_of_button;

	INIT_DELAYED_WORK(&ts->work, synaptics_ts_work_func);
	INIT_DELAYED_WORK(&ts->button_lock_work,
	...
	if (likely(ts->pdata->use_irq && client->irq)) {
		...
		ret =
		    request_threaded_irq(client->irq, synaptics_ts_irq_handler,
		...
		if (ret < 0) {
			ts->pdata->use_irq = 0;
			SYNAPTICS_ERR_MSG
			    ("request_irq failed. use polling mode\n");
		} else {
			if (synaptics_rmi4_i2c_debug_mask &
			    SYNAPTICS_RMI4_I2C_DEBUG_FUNC_TRACE)
	...
	ret =
	    sysfs_create_group(&client->dev.kobj,
			       &synaptics_ts_attribute_group);
	if (ret < 0) {
		SYNAPTICS_ERR_MSG
		    ("Synaptics touchscreen sysfs register failed\n");
...
static void synaptics_ts_suspend_func(struct synaptics_ts_data *ts)
{
	...
	if (report_enable) {
		SYNAPTICS_INFO_MSG
		    ("Release all pressed event before touch power off\n");
		input_sync(ts->input_dev);

		/* Reset finger & button status data */
		memset(ts->finger_prestate, 0x0,
		       sizeof(char) * ts->pdata->num_of_finger);
		memset(ts->button_prestate, 0x0,
		       sizeof(char) * ts->pdata->num_of_button);
	}
...
MODULE_DESCRIPTION
    ("Synaptics RMI4 protocol based I2C interface Touchscreen Driver");
MODULE_LICENSE("GPL");
