	...
	if (atomic_read(&acc->enabled)) {
		...
		if (atomic_read(&acc->enabled))
			hrtimer_start(&acc->htimer,
				      ns_to_ktime(MS_TO_NS(poll_interval_ms)),
				      HRTIMER_MODE_REL);
	}
				    ...
				    u8 reg_address, u8 mask, u8 new_bit_values)
{
	...
	if (!(err < 0)) {
		...
		err =
		    k3dh_acc_register_write(acc, buf, reg_address, updated_val);
	}
	return err;
}
...
static int k3dh_acc_get_acceleration_data(struct k3dh_acc_data *acc, int *xyz)
{
	...
	k3dh_xyz[0] = xyz[0] =
	    ((acc->pdata->
	      negate_x) ? (-hw_d[acc->pdata->axis_map_x]) : (hw_d[acc->pdata->
								  axis_map_x]));
	k3dh_xyz[1] = xyz[1] =
	    ((acc->pdata->
	      negate_y) ? (-hw_d[acc->pdata->axis_map_y]) : (hw_d[acc->pdata->
								  axis_map_y]));
	k3dh_xyz[2] = xyz[2] =
	    ((acc->pdata->
	      negate_z) ? (-hw_d[acc->pdata->axis_map_z]) : (hw_d[acc->pdata->
								  axis_map_z]));
...
static int k3dh_acc_enable(struct k3dh_acc_data *acc)
{
	...
	if (atomic_cmpxchg(&acc->enabled, 0, 1) == 0) {
		...
		hrtimer_start(&acc->htimer,
			      ns_to_ktime(MS_TO_NS(acc->pdata->poll_interval)),
			      HRTIMER_MODE_REL);
	}
...
static long k3dh_acc_misc_ioctl(struct file *file, unsigned int cmd,
				unsigned long arg)
#else
...
#endif
{
	...
	case K3DH_ACC_IOCTL_SET_CTRL_REG3:
		...
		bit_values = buf[0];
		err =
		    k3dh_acc_register_update(acc, (u8 *) arg, reg_address, mask,
					     bit_values);
		if (err < 0)
			return err;

		acc->resume_state[RES_CTRL_REG3] =
		    ((mask & bit_values) |
	...
	case K3DH_ACC_IOCTL_SET_CTRL_REG6:
		...
		acc->resume_state[RES_CTRL_REG6] =
		    ((mask & bit_values) |
...
static void k3dh_acc_input_work_func(struct work_struct *work)
{
	...
	acc =
	    container_of((struct work_struct *)work, struct k3dh_acc_data,
			 hr_work);
	if (atomic_read(&acc->enabled)) {
...
{
	struct k3dh_acc_data *acc =
	    container_of(timer, struct k3dh_acc_data, htimer);
	DEV_INFO(DEBUG_FUNC_TRACE, "%s(%d)\n", __func__, __LINE__);
...
{
	acc->pdata->poll_interval =
	    max(acc->pdata->poll_interval, acc->pdata->min_interval);
...
int k3dh_update_delay(void)
{
	...
	DEV_INFO(DEBUG_INTR_INFO, "%s(%d) : acc_poll(%d)\n", __func__, __LINE__,
		 k3dh_acc_misc_data->poll_ms);

	k3dh_acc_misc_data->pdata->poll_interval = k3dh_acc_misc_data->poll_ms;

	return k3dh_acc_update_odr(k3dh_acc_misc_data,
				   k3dh_acc_misc_data->pdata->poll_interval);
}

// I-project Middleware Interface
static ssize_t k3dh_show_enable(struct device *dev,
...
static ssize_t k3dh_store_enable(struct device *dev,
...
static DEVICE_ATTR(enable, S_IRUGO | S_IWUSR, k3dh_show_enable,
		   k3dh_store_enable);

static ssize_t k3dh_show_delay(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n",
...
static ssize_t k3dh_store_delay(struct device *dev,
				struct device_attribute *attr, const char *buf,
				size_t count)
{
	int ret;
...
static DEVICE_ATTR(poll_delay, S_IRUGO | S_IWUSR, k3dh_show_delay,
		   k3dh_store_delay);
...
static ssize_t k3dh_show_report_cnt(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	DEV_INFO(DEBUG_SYSFS_INFO, "%s(%d) : report_cnt=%d\n", __func__,
		 __LINE__, report_cnt);
	return sprintf(buf, "%d\n",
		       (atomic_read(&k3dh_acc_misc_data->enabled) ? report_cnt :
			-1));
}

static ssize_t k3dh_show_x(struct device *dev, struct device_attribute *attr,
			   char *buf)
{
	DEV_INFO(DEBUG_SYSFS_INFO, "%s(%d) : x=%d\n", __func__, __LINE__,
		 k3dh_xyz[0]);
	return sprintf(buf, "%d\n",
		       (atomic_read(&k3dh_acc_misc_data->enabled) ? k3dh_xyz[0]
			: -1));
}

static ssize_t k3dh_show_y(struct device *dev, struct device_attribute *attr,
			   char *buf)
{
	DEV_INFO(DEBUG_SYSFS_INFO, "%s(%d) : y=%d\n", __func__, __LINE__,
		 k3dh_xyz[1]);
	return sprintf(buf, "%d\n",
		       (atomic_read(&k3dh_acc_misc_data->enabled) ? k3dh_xyz[1]
			: -1));
}

static ssize_t k3dh_show_z(struct device *dev, struct device_attribute *attr,
			   char *buf)
{
	...
	DEV_INFO(DEBUG_SYSFS_INFO, "%s(%d) : z=%d\n", __func__, __LINE__,
		 k3dh_xyz[2]);
	return sprintf(buf, "%d\n",
		       (atomic_read(&k3dh_acc_misc_data->enabled) ? k3dh_xyz[2]
			: -1));
}
...
static ssize_t k3dh_show_calibration(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", calibration_status);
}

static ssize_t k3dh_store_calibration(struct device *dev,
...
static DEVICE_ATTR(calibration, S_IRUGO | S_IWUSR, k3dh_show_calibration,
		   k3dh_store_calibration);
#endif

static DEVICE_ATTR(acc_cal, S_IRUGO | S_IWUSR, k3dh_show_calibration,
		   k3dh_store_calibration);
			  ...
			  const struct i2c_device_id *id)
{
	...
	if (acc->pdata->gpio_int1 > 0) {
		...
		acc->irq1_work_queue =
		    create_singlethread_workqueue("k3dh_acc_wq1");
	...
	if (acc->pdata->gpio_int2 > 0) {
		...
		acc->irq2_work_queue =
		    create_singlethread_workqueue("k3dh_acc_wq2");
	...
	if (acc->pdata->gpio_int1 > 0) {
		...
		DEV_INFO(DEBUG_FUNC_TRACE,
			 "%s(%d)[%s] : irq1[%d] mapped on gpio[%d]\n", __func__,
			 __LINE__, K3DH_ACC_DEV_NAME, acc->irq1,
			 acc->pdata->gpio_int1);
	}
	...
	if (acc->pdata->gpio_int2 > 0) {
		...
		DEV_INFO(DEBUG_FUNC_TRACE,
			 "%s(%d)[%s] : irq1[%d] mapped on gpio[%d]\n", __func__,
			 __LINE__, K3DH_ACC_DEV_NAME, acc->irq2,
			 acc->pdata->gpio_int2);
	}
#endif
	...
	if (acc->irq1 > 0) {
		DEV_INFO(DEBUG_FUNC_TRACE,
			 "%s(%d) : call request_irq for irq1..\n", __func__,
			 __LINE__);
		err =
		    request_threaded_irq(acc->irq1, NULL, k3dh_acc_isr1,
	...
	if (acc->irq2 > 0) {
		DEV_INFO(DEBUG_FUNC_TRACE,
			 "%s(%d) : call request_irq for irq2..\n", __func__,
			 __LINE__);
		err =
		    request_irq(acc->irq2, k3dh_acc_isr2, IRQF_TRIGGER_RISING,
				"k3dh_irq2", acc);
		if (err < 0) {
...
#ifdef K3DH_USER_CALIBRATION
	err =
	    device_create_file(k3dh_acc_misc_device.this_device,
