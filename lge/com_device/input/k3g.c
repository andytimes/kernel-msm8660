				...
				struct k3g_t *data, int total_read)
{
	...
	data->y =
	    (pdata->
	     negate_y) ? (-tmp_xyz[pdata->axis_map_y]) : (tmp_xyz[pdata->
								  axis_map_y]);
	data->z =
	    (pdata->
	     negate_z) ? (-tmp_xyz[pdata->axis_map_z]) : (tmp_xyz[pdata->
								  axis_map_z]);
	data->x =
	    (pdata->
	     negate_x) ? (-tmp_xyz[pdata->axis_map_x]) : (tmp_xyz[pdata->
								  axis_map_x]);
...
static int k3g_report_gyro_values(struct k3g_data *k3g_data)
{
	...
	if (res >= 31 - k3g_data->entries) {
		...
		if (DEBUG_DEV_STATUS & debug_mask)
			printk(KERN_INFO
			       "[k3g] call restart_fifo, entries=%d\n",
			       k3g_data->entries);
			      ...
			      size_t size)
{
	...
	if (new_enable) {
		if (pdata->power_on) {
			if (DEBUG_FUNC_TRACE & debug_mask)
				printk(KERN_INFO "%s: line %d, call power_on",
				       __func__, __LINE__);
		...
		if (k3g_data->interruptible) {
			...
			if (DEBUG_FUNC_TRACE & debug_mask)
				printk(KERN_INFO
				       "%s: line %d, enable interrupt\n",
				       __func__, __LINE__);
		} else {
			set_polling_delay(k3g_data, 0);
			hrtimer_start(&k3g_data->timer,
				      k3g_data->polling_delay,
				      HRTIMER_MODE_REL);
			if (DEBUG_FUNC_TRACE & debug_mask)
				printk(KERN_INFO
				       "%s: line %d, set_polling_delay\n",
				       __func__, __LINE__);
		}
		report_cnt = 0;
	} else {
...
static long k3g_misc_ioctl(struct file *file, unsigned int cmd,
			   unsigned long arg)
{
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	if (DEBUG_DEBUG_SYSFS & debug_mask) {
		if (k3g_data->enable)
			printk(KERN_INFO "%s: %d, %d, %d\n", __func__,
			       k3g_gyro_data[0], k3g_gyro_data[1],
			       k3g_gyro_data[2]);
		else
			printk(KERN_INFO "%s: %d, %d, %d\n", __func__, -1, -1,
			       -1);
	...
	if (k3g_data->enable)
		sprintf(strbuf, "%d %d %d", k3g_gyro_data[0], k3g_gyro_data[1],
			k3g_gyro_data[2]);
	else
				 ...
				 size_t size)
{
	...
	struct k3g_platform_data *pdata;
	int out_nost_x[NUM_SAMPLE], out_nost_y[NUM_SAMPLE],
	    out_nost_z[NUM_SAMPLE];
	int avg_out_nost_x = 0, avg_out_nost_y = 0, avg_out_nost_z =
	    0, sum_nost_x = 0, sum_nost_y = 0, sum_nost_z = 0;
	int out_st_x[NUM_SAMPLE], out_st_y[NUM_SAMPLE], out_st_z[NUM_SAMPLE];
	int avg_out_st_x = 0, avg_out_st_y = 0, avg_out_st_z = 0, sum_st_x =
	    0, sum_st_y = 0, sum_st_z = 0;
	...
	if (err < 0) {
		err = -EIO;
		printk(KERN_ERR
		       "[%s, %d] Error during writing 0x6f on CTRL_REG1\n",
		       TAG_ST, __LINE__);
		goto exit_self_test;
	}
	...
	if (err < 0) {
		err = -EIO;
		printk(KERN_ERR
		       "[%s, %d] Error during writing 0x00 on CTRL_REG2\n",
		       TAG_ST, __LINE__);
		goto exit_self_test;
	}
	...
	if (err < 0) {
		err = -EIO;
		printk(KERN_ERR
		       "[%s, %d] Error during writing 0x00 on CTRL_REG3\n",
		       TAG_ST, __LINE__);
		goto exit_self_test;
	}
	...
	if (err < 0) {
		err = -EIO;
		printk(KERN_ERR
		       "[%s, %d] Error during writing 0xa0 on CTRL_REG4\n",
		       TAG_ST, __LINE__);
		goto exit_self_test;
	}
	...
	if (err < 0) {
		err = -EIO;
		printk(KERN_ERR
		       "[%s, %d] Error during writing 0x02 on CTRL_REG5\n",
		       TAG_ST, __LINE__);
		goto exit_self_test;
	}
	...
	while (i < NUM_SAMPLE) {
		tmp1 = 0;
		tmp2 = 0;
		if (i2c_smbus_read_byte_data(k3g_data->client, STATUS_REG) &
		    ZYXDA_MASK) {

			if (DEBUG_DEV_STATUS & debug_mask)
				printk(KERN_INFO
				       "[%s, %d] NOST read data #%d\n", TAG_ST,
				       __LINE__, i);
			tmp1 =
			    i2c_smbus_read_byte_data(k3g_data->client, OUT_X_L);
			tmp2 =
			    i2c_smbus_read_byte_data(k3g_data->client, OUT_X_H);
			out_nost_x[i] = (tmp2 << 8) | tmp1;
			if (0x8000 & out_nost_x[i])
				out_nost_x[i] =
				    (0x10000 - out_nost_x[i]) * (-1);

			tmp1 =
			    i2c_smbus_read_byte_data(k3g_data->client, OUT_Y_L);
			tmp2 =
			    i2c_smbus_read_byte_data(k3g_data->client, OUT_Y_H);
			out_nost_y[i] = (tmp2 << 8) | tmp1;
			if (0x8000 & out_nost_y[i])
				out_nost_y[i] =
				    (0x10000 - out_nost_y[i]) * (-1);

			tmp1 =
			    i2c_smbus_read_byte_data(k3g_data->client, OUT_Z_L);
			tmp2 =
			    i2c_smbus_read_byte_data(k3g_data->client, OUT_Z_H);
			out_nost_z[i] = (tmp2 << 8) | tmp1;
			if (0x8000 & out_nost_z[i])
				out_nost_z[i] =
				    (0x10000 - out_nost_z[i]) * (-1);

			i++;
		} else {
			if (DEBUG_DEV_STATUS & debug_mask)
				printk(KERN_INFO
				       "[%s, %d] NOST ZYXDA ready bit is not set\n",
				       TAG_ST, __LINE__);
			mdelay(1);
		}
	}
	...
	for (i = 0; i < NUM_SAMPLE; i++) {
		...
		if (DEBUG_DEV_STATUS & debug_mask) {
			printk(KERN_INFO
			       "[%s, %d] sum_nost_x=(%d), out_nost_x[%d]=(%d)\n",
			       TAG_ST, __LINE__, sum_nost_x, i, out_nost_x[i]);
			printk(KERN_INFO
			       "[%s, %d] sum_nost_y=(%d), out_nost_y[%d]=(%d)\n",
			       TAG_ST, __LINE__, sum_nost_y, i, out_nost_y[i]);
			printk(KERN_INFO
			       "[%s, %d] sum_nost_z=(%d), out_nost_z[%d]=(%d)\n",
	...
	if (DEBUG_DEV_STATUS & debug_mask)
		printk(KERN_INFO
		       "[%s, %d] avg_out_nost_x(%d), avg_out_nost_y(%d), avg_out_nost_z(%d)\n",
		       TAG_ST, __LINE__, avg_out_nost_x, avg_out_nost_y,
		       avg_out_nost_z);
	...
	while (j < NUM_SAMPLE) {
		...
		if (i2c_smbus_read_byte_data(k3g_data->client, STATUS_REG) &
			...
			if (DEBUG_DEV_STATUS & debug_mask)
				printk(KERN_INFO
				       "[%s, %d] ST ZYXDA ready bit is not set\n",
				       TAG_ST, __LINE__);
			mdelay(1);
		}
	}
	...
	for (j = 0; j < NUM_SAMPLE; j++) {
		...
		if (DEBUG_DEV_STATUS & debug_mask) {
			printk(KERN_INFO
			       "[%s, %d] sum_st_x=(%d), out_st_x[%d]=(%d)\n",
			       TAG_ST, __LINE__, sum_st_x, j, out_st_x[j]);
			printk(KERN_INFO
			       "[%s, %d] sum_st_y=(%d), out_st_y[%d]=(%d)\n",
			       TAG_ST, __LINE__, sum_st_y, j, out_st_y[j]);
			printk(KERN_INFO
			       "[%s, %d] sum_st_z=(%d), out_st_z[%d]=(%d)\n",
	...
	if (DEBUG_DEV_STATUS & debug_mask)
		printk(KERN_INFO
		       "[%s, %d] avg_out_st_x(%d), avg_out_st_y(%d), avg_ouot_st_z(%d)\n",
		       TAG_ST, __LINE__, avg_out_st_x, avg_out_st_y,
		       avg_out_st_z);
	...
	/* 4. decide pass/fail */
	{
		...
		tmpx =
		    K3G_ABS(avg_out_st_x - avg_out_nost_x) * SENSITIVITY / 1000;
		if (DEBUG_DEV_STATUS & debug_mask)
			printk(KERN_INFO
			       "[%s, %d] tmpx(%d)=(%d*0.001)*|avg_out_st_x(%d)-avg_out_nost_x(%d)|\n",
			       TAG_ST, __LINE__, tmpx, SENSITIVITY,
			       avg_out_st_x, avg_out_nost_x);
		if (MIN_ST_X <= tmpx && tmpx <= MAX_ST_X) {
			if (DEBUG_DEV_STATUS & debug_mask)
				printk(KERN_ERR
				       "[%s, %d] Pass, ST_X, (%d)<=(%d)<=(%d)\n",
				       TAG_ST, __LINE__, MIN_ST_X, tmpx,
				       MAX_ST_X);
			ret = true;
		} else {
			printk(KERN_ERR
			       "[%s, %d] False, ST_X, (%d)<=(%d)<=(%d)\n",
		...
		tmpy =
		    K3G_ABS(avg_out_st_y - avg_out_nost_y) * SENSITIVITY / 1000;
		if (DEBUG_DEV_STATUS & debug_mask)
			printk(KERN_INFO
			       "[%s, %d] tmpy(%d)=(%d*0.001)*|avg_out_st_y(%d)-avg_out_nost_y(%d)|\n",
			       TAG_ST, __LINE__, tmpy, SENSITIVITY,
			       avg_out_st_y, avg_out_nost_y);
		if (MIN_ST_Y <= tmpy && tmpy <= MAX_ST_Y) {
			if (DEBUG_DEV_STATUS & debug_mask)
				printk(KERN_ERR
				       "[%s, %d] Pass, ST_Y, (%d)<=(%d)<=(%d)\n",
				       TAG_ST, __LINE__, MIN_ST_Y, tmpy,
				       MAX_ST_Y);
			ret = true;
		} else {
			printk(KERN_ERR
			       "[%s, %d] False, ST_Y, (%d)<=(%d)<=(%d)\n",
		...
		tmpz =
		    K3G_ABS(avg_out_st_z - avg_out_nost_z) * SENSITIVITY / 1000;
		if (DEBUG_DEV_STATUS & debug_mask)
			printk(KERN_INFO
			       "[%s, %d] tmpz(%d)=(%d*0.001)*|avg_out_st_z(%d)-avg_out_nost_z(%d)|\n",
			       TAG_ST, __LINE__, tmpz, SENSITIVITY,
			       avg_out_st_z, avg_out_nost_z);
		if (MIN_ST_Z <= tmpz && tmpz <= MAX_ST_Z) {
			if (DEBUG_DEV_STATUS & debug_mask)
				printk(KERN_ERR
				       "[%s, %d] Pass, ST_Z, (%d)<=(%d)<=(%d)\n",
				       TAG_ST, __LINE__, MIN_ST_Z, tmpz,
				       MAX_ST_Z);
			ret = true;
		} else {
			printk(KERN_ERR
			       "[%s, %d] False, ST_Z, (%d)<=(%d)<=(%d)\n",
...
exit_self_test:
	...
	if (err < 0) {
		err = -EIO;
		printk(KERN_ERR
		       "[%s, %d] exit: Error during writing 0x02 on CTRL_REG5\n",
		       TAG_ST, __LINE__);
		//goto exit;
	}
...
static ssize_t k3g_show_report_cnt(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	struct k3g_data *k3g_data = dev_get_drvdata(dev);
...
static DEVICE_ATTR(gyro_data, /*S_IRUGO | S_IWUSR | S_IWGRP, */
		   S_IRUGO | S_IWUSR,
		   k3g_show_gyro_data, NULL);
static DEVICE_ATTR(gyro_cnt, /*S_IRUGO | S_IWUSR | S_IWGRP, */
		   S_IRUGO | S_IWUSR,
		     ...
		     const struct i2c_device_id *devid)
{
	...
	if (pdata->power_on) {
		...
		if (DEBUG_FUNC_TRACE & debug_mask)
			printk(KERN_INFO "%s: line %d, call power_on", __func__,
			       __LINE__);
		pdata->power_on(1 << SENSOR_TYPE_GYROSCOPE);
		mdelay(1);
	}
	...
	if (pdata->power_off) {
		if (DEBUG_FUNC_TRACE & debug_mask)
			printk(KERN_INFO "%s: line %d, call power_off",
			       __func__, __LINE__);
...
#ifdef SELF_TEST_ENABLED
	if (device_create_file(&input_dev->dev, &dev_attr_self_test) < 0) {
		pr_err("Failed to create device file(%s)!\n",
	...
	if (device_create_file(&input_dev->dev, &dev_attr_gyro_data) < 0) {
		pr_err("Failed to create device file(%s)!\n",
		       dev_attr_gyro_data.attr.name);
		goto err_device_create_file4;
	}
#ifdef CONFIG_MACH_LGE_I_BOARD_VZW
	if (device_create_file(&input_dev->dev, &dev_attr_X) < 0) {
		pr_err("Failed to create device file(%s)!\n",
		       dev_attr_X.attr.name);
		goto err_device_create_file5;
	}

	if (device_create_file(&input_dev->dev, &dev_attr_Y) < 0) {
		pr_err("Failed to create device file(%s)!\n",
		       dev_attr_Y.attr.name);
		goto err_device_create_file6;
	}

	if (device_create_file(&input_dev->dev, &dev_attr_Z) < 0) {
		pr_err("Failed to create device file(%s)!\n",
		       dev_attr_Z.attr.name);
		goto err_device_create_file7;
	}
#endif

	if (device_create_file(&input_dev->dev, &dev_attr_gyro_cnt) < 0) {
		pr_err("Failed to create device file(%s)!\n",
	...
	if (err < 0) {
		dev_err(&client->dev, "misc device register failed\n");
		goto err_device_create_file2;
	}
#endif
...
err_device_create_file7:
	device_remove_file(&input_dev->dev, &dev_attr_Z);

err_device_create_file6:
	device_remove_file(&input_dev->dev, &dev_attr_Y);

err_device_create_file5:
	device_remove_file(&input_dev->dev, &dev_attr_X);
	...
	if (k3g_data->enable) {
		...
		if (!k3g_data->interruptible)
			hrtimer_start(&k3g_data->timer,
				      ...
				      HRTIMER_MODE_REL);
		err =
		    i2c_smbus_write_i2c_block_data(client, CTRL_REG1 | AC,
						   sizeof(k3g_data->ctrl_regs),
						   k3g_data->ctrl_regs);
		mutex_unlock(&k3g_data->lock);
	}
