...
extern void mcsdl_delay_test(INT32 nCount);
...
void mms100_download(void) 
{
	...
	 gpio_request(MELFAS_MMS144_I2C_SDA, "Melfas_I2C_SDA");
	gpio_request(MELFAS_MMS144_I2C_SCL, "Melfas_I2C_SCL");
	gpio_request(MELFAS_MMS144_I2C_INT_GPIO, "Melfas_I2C_INT");
	 gpio_tlmm_config(GPIO_CFG
			    (MELFAS_MMS144_I2C_SDA, 0, GPIO_CFG_OUTPUT,
			     GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	gpio_tlmm_config(GPIO_CFG
			  (MELFAS_MMS144_I2C_SCL, 0, GPIO_CFG_OUTPUT,
			   GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	gpio_tlmm_config(GPIO_CFG
			  (MELFAS_MMS144_I2C_INT_GPIO, 0, GPIO_CFG_INPUT,
			   GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	    ...
	    ret = mms100_ISC_download_binary_data();
			...
			if (!ret)
			      ...
			      (MELFAS_MMS144_I2C_SDA, 1, GPIO_CFG_OUTPUT,
			       GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
			      GPIO_CFG_ENABLE);
	gpio_tlmm_config(GPIO_CFG
			  (MELFAS_MMS144_I2C_SCL, 1, GPIO_CFG_OUTPUT,
			   GPIO_CFG_NO_PULL, GPIO_CFG_8MA), GPIO_CFG_ENABLE);
	 gpio_free(MELFAS_MMS144_I2C_SDA);
	gpio_free(MELFAS_MMS144_I2C_SCL);
	gpio_free(MELFAS_MMS144_I2C_INT_GPIO);
	...
	MELFAS_DISABLE_WATCHDOG_TIMER_RESET();	// Disable Baseband watchdog timer
	    ...
	    //------------------------
			...
			 if (i < 2)	// 0: core, 1: private custom
			{
				    ...
				    mms100_ISC_download((const UINT8 *)
							MELFAS_binary,
							(const UINT16)
